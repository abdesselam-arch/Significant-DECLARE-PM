"""
DeclareMiner_SepsisFST.py  —  DeclareMiner FST Baseline: Sepsis (Option A)
===========================================================================
DeclareMiner-style discriminative baseline using the principled per-constraint
sample-size-scaled effect-size threshold (Option A / FST formulation) instead
of a globally calibrated hard threshold.
...
OPTION A — CONSTRAINT-SPECIFIC EFFECT-SIZE THRESHOLD (FST)
-----------------------------------------------------------
τ*(c) = tau_effect × √(n_ref / n_harm(c))

where n_harm(c) = 2 * n_app0 * n_app1 / (n_app0 + n_app1) is the harmonic mean
of applicable trace counts per class.  This scales the required confidence
difference with estimation noise, anchored to a user-chosen practical effect
size at n_ref applicable traces.

Key properties:
    τ*(c) is constraint-specific, not a single global threshold.
    tau_effect has no Type I error interpretation — it is the minimum
    practically meaningful confidence difference at the reference sample size.
    Computed ONCE on the original log and held FIXED across all doubly-null
    replicates — recomputing per replicate would make the threshold data-adaptive
...
Decision rule (Option A / FST):
    Reject r iff  |Δconf̂(r)| ≥ τ*(r)
                  AND (conf(r, L_0) ≥ τ_min OR conf(r, L_1) ≥ τ_min)

where τ*(r) = tau_effect × √(n_ref / n_harm(r)), computed once on the real log.
...
------------
    ...
    declareminer_fst_tau_c.csv                  Per-constraint threshold τ*(c).

Version : 3.0  (Option A: constraint-specific effect-size threshold)
...
def run_declareminer(
    ...
    """
    Execute the full DeclareMiner FST (Option A) pipeline on Sepsis.
    ...
    Key return fields:
        'tau_c'        : (m,) float64 array — per-constraint thresholds,
                         computed on the real log.  Pass this to
                         run_declareminer_on_doubly_null_log unchanged.
        'eligible'     : (m,) bool — FROZEN eligibility mask for null replicates.
        'tau_effect'   : the practical effect size used.
        'n_ref'        : reference applicable-trace count.
        'n_floor'      : hard exclusion floor.
    ...
    # Sort by z-score descending (= Δconf / τ*(c), analogous to DRVA's Ediff ranking)
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "DeclareMiner — Sample-Size-Scaled Effect-Size Threshold Baseline — Sepsis. "
