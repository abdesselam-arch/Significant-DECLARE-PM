#!/usr/bin/env python3
"""
rq2_Production_BISE.py  —  RQ2 Specification Quality Degradation: Production  [DvM BISE 2025]
==============================================================================================
...
RESEARCH QUESTION
-----------------
RQ2 (DvM BISE 2025 / Production): As signal is progressively corrupted along BOTH
noise axes simultaneously, how does DvM BISE 2025 degrade across all four output sets:
...
-----------------------------------------------
    sel_features   Fisher+coverage selection
    ripper_rules   RipperK rules trained on sel submatrix
    dt_rules       DT rules trained on sel submatrix
    ripper+dt      ripper_rules u dt_rules

All four sets computed from ONE DvM evaluation per cell or null replicate.
...
------------
    sel_features/
        ...
        rq2_dvm_bise2025_joint_<metric>_sel.csv
    rules/
        rq2_dvm_bise2025_joint_metrics_{ripper/dt/union}.csv
        rq2_dvm_bise2025_joint_null_counts_{ripper/dt/union}.json
        rq2_dvm_bise2025_joint_results_rules.json
        rq2_dvm_bise2025_joint_<metric>_{ripper/dt/union}.csv
...
# DvM BISE 2025 Production module
...
RQ2_OUTPUT_DIR = "RQ2_Production_DvM_BISE2025"
...
def _dvm_all_sets_on_X(y, X_struct, feat_names, coverage_threshold, fisher_eps=1e-10):
    """
    Apply Fisher+coverage + rule extraction to a precomputed feature matrix.
    Returns all four output sets from a SINGLE pass over (X_struct, y).
    """
...
def compute_reference_metrics(S_pert, S_orig_full, S_orig_rq2) -> dict:
    ...
    if R_obs == 0:
        precision = float('nan'); fdr_ref = float('nan'); estimable = False
    else:
        precision = TP / R_obs; fdr_ref = FP / R_obs; estimable = True
...
def _compute_fdr_from_null(null_counts, R_obs, alpha_nominal):
    ...
    if R_obs == 0:
        return {
            ...
            'FWER_emp': float(np.mean(null_counts > 0)), 'controls_FDR': None,
            'estimable': False, 'skipped_reason': 'R_obs=0: FDR_emp undefined',
        }
...
def analyze_joint_cell(
    ...
    S_orig_full, S_orig_rq2, coverage_threshold, is_anchor_bootstrap=False,
) -> dict:
    """Full analysis for one (eps, rho) cell — all four output sets."""
    ...
    for retry in range(MAX_RETRIES + 1):
        ...
        if draw['R_obs_sel'] > 0:
            n_retries = retry; break
        if retry < MAX_RETRIES:
            print(f"  [eps={eps:.2f}  rho={rho:.2f}]  R_obs_sel=0 — retrying...", flush=True)
        else:
            n_retries = retry

    SET_KEYS = ('sel', 'ripper', 'dt', 'union')
    S_pert_all = {
        'sel': draw['S_pert_sel'], 'ripper': draw['S_pert_ripper'],
        'dt':  draw['S_pert_dt'],  'union':  draw['S_pert_union'],
    }
    R_obs_all = {
        'sel': draw['R_obs_sel'], 'ripper': draw['R_obs_ripper'],
        'dt':  draw['R_obs_dt'],  'union':  draw['R_obs_union'],
    }

    _S_rq2_all = S_pert_all if (is_anchor_bootstrap or S_orig_rq2 is None) else S_orig_rq2
...
def run_original_data_dvm() -> dict:
    """
    Run DvM BISE 2025 on the original (unperturbed) Production log.
    Computes all four output sets in a single pipeline execution.
    """
    ...
    for k, label in [('sel', 'sel_features'), ('ripper', 'ripper_rules (DS)'),
                     ('dt', 'dt_rules (DS)'),  ('union', 'ripper+dt (DS)')]:
        print(f"  |S_orig_full| {label:20s} = {len(S_orig_full[k]):,}")
...
def run_joint_grid(
    ...
    """Run 7x7 joint noise grid. Returns (all_results, S_orig_rq2)."""
    cov_t = int(DVM_CONFIG.get("coverage_threshold", DVM_COVERAGE_THRESHOLD))
    ...
    # Step 0: structural logs + X per rho row
    print("\n  [Step 0] Precomputing structural logs and X matrices...", flush=True)
    struct_logs = {}
    X_cache     = {}
    for rho_idx, rho in enumerate(JOINT_STRUCT_LEVELS):
        if rho == 0.0:
            ...
            rho_seed = BASE_SEED + rho_idx * 13
            t_rho    = time.time()
            sl       = apply_structural_noise(case_data_orig, rho, rho_seed)
            struct_logs[rho] = sl
            ...
            print(f"    rho={rho:.2f}  -> X in {time.time()-t_rho:.1f}s  "
                  f"(shape: {X_struct.shape})", flush=True)

    # Step 1: (0,0) anchor -> S_orig_rq2
    print("\n  [Step 1] Running (0,0) anchor -> S_orig_rq2...", flush=True)
    ...
    S_orig_rq2 = {k: frozenset(anchor_result['S_pert_all_raw'][k])
                  for k in ('sel', 'ripper', 'dt', 'union')}
    for k in ('sel', 'ripper', 'dt', 'union'):
        print(f"    S_orig_rq2[{k}]: {len(S_orig_rq2[k]):,}")
...
def build_metrics_table_for_set(all_results, set_key) -> pd.DataFrame:
    ...
    for res in all_results:
        ...
        rows.append({
            'eps': res['eps'], 'rho': res['rho'],
            'method': METHOD_DM, 'output_set': set_key,
            'R_obs': res['R_obs_all'][set_key],
            'FDR_ref': rm['FDR_ref'], 'Precision': rm['Precision'],
            'Recall': rm['Recall'], 'F1': rm['F1'],
            'TP': rm['TP'], 'FP': rm['FP'], 'FN': rm['FN'], 'R_full': rm['R_full'],
            'Jaccard_rq2': rm['Jaccard_rq2'], 'Jaccard_full': rm['Jaccard_full'],
            'Gained_rq2': rm['Gained_rq2'], 'Lost_rq2': rm['Lost_rq2'],
            'reliable': rm['reliable'],
            'FP_over_Rfull': rm['FP_over_Rfull'], 'FN_over_Rfull': rm['FN_over_Rfull'],
            'FDR_emp': fdr['FDR_emp'],
            ...
            'B_null': res['B_null'], 'is_anchor': res.get('is_anchor', False),
...
def _print_joint_summary(all_results) -> None:
    ...
    for eps in JOINT_LABEL_LEVELS:
        ...
        for rho in JOINT_STRUCT_LEVELS:
            res = next((r for r in all_results if r['eps'] == eps and r['rho'] == rho), None)
            v   = res['ref_metrics_all']['sel']['FDR_ref'] if res else float('nan')
...
def main():
    print("\n" + "=" * 100)
    print("RQ2 — SPECIFICATION QUALITY DEGRADATION: PRODUCTION  [DvM BISE 2025]")
    ...
    print(f"\n{'='*100}")
    print("RQ2 — PRODUCTION DvM BISE 2025 COMPLETE  (Joint Noise, four output sets)")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="RQ2 Degradation — Production (DvM BISE 2025, four output sets)"
    )
