#!/usr/bin/env python3
"""
rq2_ProductionDRVA.py  —  RQ2 Specification Quality Degradation: DRVA Baseline — Production
=============================================================================================
...
REFERENCE SET
-------------
  S_orig_drva  — DRVA at (ε=0, ρ=0) on the original Production log.
...
RQ2_OUTPUT_DIR  = "RQ2_Production_DRVA"
...
PI_DRVA_REAL = 2_000   # π for real-data perturbed levels
...
def compute_reference_metrics(
    S_pert:       frozenset,   # DRVA discoveries at this cell
    S_orig_drva:  frozenset,   # DRVA at (ε=0, ρ=0) — ground truth and baseline
) -> dict:
    """
...
def analyze_joint_cell(
    ...
    (0,0) cell (is_anchor_bootstrap=True): S_orig_drva is set from own output.
...
def run_original_data() -> dict:
    """
    Load data and run DRVA on the original (unperturbed) Production log.
    Returns case_data, candidates_all, S_orig_drva, holds_orig.
    """
    print("\n" + "=" * 100)
    print("SECTION 1 — ORIGINAL DATA RUN  (DRVA on Production)")
    print(f"  π={PI_DRVA_REAL}  α_DRVA={ALPHA_DRVA}  hierarchical_pruning=False")
    print("=" * 100)
    ...
    # Precompute holds for original data (reused for ρ=0 row)
    print("  Precomputing holds for original data...", flush=True)
    with _suppress():
        holds_orig = compute_holds_by_case_batch(case_data, candidates_all)
...
def run_joint_grid(
    ...
    Architecture:
    Step 0 — Precompute structural logs and holds per ρ row (7 total).
    ...
    print(f"  PI_DRVA_NULL={PI_DRVA_NULL}  PI_DRVA_REAL={PI_DRVA_REAL}")
    print(f"  α_DRVA={ALPHA_DRVA}  n_jobs={n_jobs}")
    print("=" * 100)
    ...
    for rho_idx, rho in enumerate(JOINT_STRUCT_LEVELS):
        if rho == 0.0:
            ...
            print(
                f"    ρ={rho:.2f}  → structural log + holds in {time.time()-t_rho:.1f}s",
                flush=True,
            )
    ...
    print(f"\n  Grid complete: {len(all_results)} cells  |  "
...
def main():
    print("\n" + "=" * 100)
    print("RQ2 — SPECIFICATION QUALITY DEGRADATION: DRVA BASELINE — PRODUCTION")
    ...
    print(f"\n{'='*100}")
    print("RQ2 — DRVA PRODUCTION COMPLETE")
    print(f"{'='*100}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="RQ2 Degradation — DRVA Baseline, Production (joint noise B.3)"
    )
    parser.add_argument(
