"""
drva_Production_parallel.py  —  DRVA Baseline: Production Event Log
====================================================================
...
(Algorithm 1, Cecconi et al. 2021) on the Production event log, using the
...
def run_drva(
    ...
    print(f"   L_B (Regular, class 0): {n_B:,}")
...
def save_outputs(drva_out: dict) -> None:
    ...
    full_json = {
        ...
        'scientific_description': {
            'null_hypothesis': (
                'No difference in Confidence between Deviant (re-submission required) '
                'and Regular (Normal) variants for rule r.'
            ),
            'permutation_scheme': (
    ...
    lines.append(f"  Negative (Regular-dominant): {n_neg:,}")
    lines.append("")
    lines.append("=" * 120)
...
def run_drva_on_doubly_null_log(
    ...
    Args:
        ...
        holds_all:       Precomputed holds dict on the null log (optional speedup).
                         When provided, the (n, m) encoding matrix is built directly
                         from the dict, avoiding a full re-evaluation of every
                         (pattern, trace) pair via evaluate_pattern_fast.
...
def main():
    parser = argparse.ArgumentParser(
        description="DRVA Baseline — Production  (shared M_all design, v2.1)"
    )
