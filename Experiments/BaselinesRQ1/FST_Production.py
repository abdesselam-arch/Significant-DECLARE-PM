"""
FST_Production.py  —  Fisher Score Threshold (FST) Non-Statistical Baseline: Production
===================================================================================================
...
THRESHOLD SELECTION — TOP-K ALGORITHM (Candidate B)
----------------------------------------------------
τ_F* is selected by Top-k with k = K_P1_REJECTIONS = 201:

    Step 1. Compute F(c) for all c ∈ M_all on the original data.
    Step 2. Sort constraints: primary key = −F(c) (descending), secondary
            key = (constraint_type, activity_a, activity_b) (lex ascending).
            Ties are broken deterministically with no random component.
    Step 3. τ_F* = F(c_(k)), the Fisher score of the k-th constraint in the
            sorted order (1-indexed: element at 0-index k−1).
    Step 4. S^FST_orig = {c_(1), ..., c_(k)} — top k constraints.
            By construction: |S^FST_orig| = k = K_P1_REJECTIONS = 201 exactly.

Application to any log L':
    Recompute F_{L'}(c) from L's data for every c ∈ M_all.
    Accept c if F_{L'}(c) >= τ_F*.
    On null/perturbed logs |S^FST_{L'}| may differ from k — this free
    variation is what the FDR estimator measures.
...
rq1_Production_parallel.py imports continue to work without change:
...
Version : 3.0  (FST — Top-k with k=201; replaces DeclareMiner v2.0)
...
OUTPUT_DIR        = "FST_Production"
os.makedirs(OUTPUT_DIR, exist_ok=True)

# ── R_obs^P1 on Production (full P1 pipeline, B_null=200) ────────────────────
# both=183 + discriminative_only=18 from three_hypothesis_houfdr_results.json.
# Retained for gap reporting only — does NOT drive the threshold.
K_P1_REJECTIONS: int = 183
...
# τ_F* = χ²_{1−α_F}(df=1) is the per-comparison critical value.
ALPHA_F: float = 0.05
...
def apply_fst_decision(
    ...
    τ_F* = χ²_{1−α_F}(df=1) is fixed across all logs; R_obs^FST varies freely.

    Parameters
    ----------
    fisher_scores : (m,) float64 — Fisher scores computed on the target log.
    tau_f_star    : float — fixed chi-squared critical value from get_fst_threshold.
...
def run_fst(
    ...
    """
    Execute the full Fisher Score Threshold pipeline on Production.
    ...
    The pipeline has five steps:
        ...
        4. Top-k threshold selection: τ_F* = F(c_(k)), k = K_P1_REJECTIONS.
    ...
    R_obs_target   : Ignored in FST — k is always K_P1_REJECTIONS = 201.
                     Retained for signature compatibility with run_declareminer.
    ...
    dict with keys required by rq1 and rq2:
        ...
        n_rejected      : int — |S^FST| = K_P1_REJECTIONS on original data.
        tau_star        : float — τ_F* (Fisher score threshold).
        k               : int — K_P1_REJECTIONS = 201.
        ...
        score_df        : pd.DataFrame — F(c) distribution for paper figure (see also above).
    ...
    print(f"  τ_F* = χ²_{{1−α_F}}(df=1)  at  α_F = {cfg.get('alpha_f', ALPHA_F)}  (fixed; not data-adaptive)")
    ...
    print("\n" + "=" * 100)
    print("FST — STEP 4: CHI-SQUARED THRESHOLD")
    print("=" * 100)
    print(f"   α_F = {alpha_f}  |  τ_F* = χ²_{{1−{alpha_f}}}(df=1) = {tau_f_star:.8f}")
    print(f"   Under H0: conf0(c)=conf1(c), F(c)~chi2(1) asymptotically.")
    print(f"   E[V_b] ≈ α_F × m ≈ {alpha_f} × {m_total} ≈ {alpha_f * m_total:.1f}  (uncontrolled FDR)")
    print(f"   F >= τ_F*: {n_above:,}   F < τ_F*: {n_below:,}")
    print(f"   Predicted FDR_emp ≈ {alpha_f * m_total / max(n_above, 1):.3f}  (if all above are null)")

    # ── Step 5: Decision ─────────────────────────────────────────────────
    print("\n" + "=" * 100)
    print(f"FST — STEP 5: ACCEPTANCE DECISION  (F(c) >= {tau_f_star:.8f})")
    print("=" * 100)
    ...
    print(f"     M_all = {m_total:,}  |  α_F = {alpha_f}  |  τ_F* = {tau_f_star:.8f}  "
          f"|  R_obs^FST = {n_rejected}  |  R_obs^P1 = {K_P1_REJECTIONS}  "
          f"|  gap = {K_P1_REJECTIONS - n_rejected}")

    return {
        'results_all':    results_all,
        'rejected':       rejected_mask,
        'n_rejected':     n_rejected,       # freely data-determined
...
def save_outputs(fst_out: dict) -> None:
    ...
    full_json = {
        ...
        'grounding': (
            ...
            f'Threshold: τ_F* = χ²_{{1−α_F}}(df=1) = {tau_f_star:.6f} at α_F = {alpha_f}.'
        ),
        'description': {
            'criterion': (
            ...
            'threshold_rule': (
                f'Fixed chi-squared critical value: τ_F* = χ²_{{1−{alpha_f}}}(df=1) = {tau_f_star:.8f}. '
                f'R_obs^FST = {n_rejected} (freely data-determined). '
                f'R_obs^P1 = {K_P1_REJECTIONS}. gap = {gap}.'
            ),
            'why_fdr_fails': (
                'Under σ∅ = σ_trace ∘ σ_label, F_null(c) ~ chi2(1) asymptotically. '
                f'E[V_b] ≈ α_F × m ≈ {alpha_f} × {m_total} ≈ {alpha_f * m_total:.1f}. '
    ...
    lines.append("Production — Deviant vs. Regular")
...
# rq1_Production_parallel.py and rq2_Production_parallel.py import by the original names.
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Fisher Score Threshold (FST) Baseline — Production  "
            "(Di Francescomarino et al. 2025 / Gu et al. 2011)"
        )
    )
    parser.add_argument(
        '--alpha-f', type=float, default=ALPHA_F,
        help=f'Per-comparison significance level for chi-squared threshold (default: {ALPHA_F})',
    )
    parser.add_argument(
    ...
    tau_star = get_fst_threshold(args.alpha_f)
    config = {
    ...
    print("Production: Deviant vs. Regular")
    print("Grounded in: Di Francescomarino et al. (2025); Gu et al. (2011)")
    print("=" * 100)
    print(f"  α_F = {config['alpha_f']}  |  τ_F* = χ²_{{1−α_F}}(df=1) = {tau_star:.8f}  (fixed; not data-adaptive)")
    print(f"  Fisher eps = {config['fisher_eps']:.2e}")
    print(f"  K_P1_REJECTIONS = {K_P1_REJECTIONS}  (gap reporting only)")
    ...
    gap = K_P1_REJECTIONS - fst_out['n_rejected']
    print(f"\n{'='*100}")
    print("FST BASELINE COMPLETE")
    print(f"{'='*100}")
    print(f"  M_all:         {fst_out['m_total']:,}")
    print(f"  α_F:           {fst_out['alpha_f']}")
    print(f"  τ_F*:          {fst_out['tau_star']:.8f}  (fixed chi-squared critical value)")
    print(f"  R_obs^FST:     {fst_out['n_rejected']:,}  (freely data-determined)")
    print(f"  R_obs^P1:      {K_P1_REJECTIONS}")
    print(f"  gap:           {gap}")
