#!/usr/bin/env python3
"""
rq1_Sepsis_BISE.py  —  RQ1 FDR Control Validity: Sepsis  [DvM BISE 2025]
=========================================================================
...
-----------------------------------------------------
    ...
    ripper+dt      ripper_rules ∪ dt_rules (unique rule strings)
...
where S is one of: sel_features, ripper_rules, dt_rules, ripper+dt_rules.

OUTPUT FILES
------------
    sel_features/
        rq1_dvm_bise2025_fdr_metrics.csv       FDR table for sel_features.
        rq1_dvm_bise2025_null_counts.csv       B_null rows of V_b (sel_features).
        rq1_dvm_bise2025_diagnostics.csv       Per-constraint Fisher scores.
        rq1_dvm_bise2025_results.json          Full results (sel_features).
    rules/
...
# DvM BISE 2025 Sepsis module — all pipeline primitives resolved here
...
RQ1_OUTPUT_DIR = "RQ1_Sepsis_DvM_BISE2025"
...
# Local config copy — can be overridden by CLI args
DVM_CONFIG = dict(_DVM_CONFIG_BASE)

# Classifier parameters for rule extraction (mirrors DvM BISE 2025 Section 7.4)
...
def _fit_rules_on_array(
    ...
    Used for both original-data rule extraction and null-replicate rule counts.
    Mirrors the final-model fitting logic in DvM _fit_and_extract_rules, but
    accepts a pre-built X_sel to avoid redundant encoding.
            ...
            for k in RIPPER_K_GRID:
                try:
                    clf = _lw_ripper.RIPPER(
                        k=k,
                        prune_size=_RIPPER_PRUNE_SIZE,
                        random_state=_RULES_RANDOM_STATE,
                    )
                    clf.fit(df, class_feat="label", pos_class=1)
                    y_pred = np.array(
                        clf.predict(df.drop(columns=["label"])), dtype=int
                    )
        ...
        for depth in DT_DEPTH_GRID:
            try:
                clf = DecisionTreeClassifier(
                    max_depth=depth, random_state=_RULES_RANDOM_STATE
                )
                clf.fit(X_sel, y)
...
def run_dm_original() -> dict:
    """
    Run the DvM BISE 2025 pipeline on the real Sepsis data.

    Computes sel_features via Fisher+coverage, then trains RIPPER and DT on
    the selected feature submatrix to extract ripper_rules and dt_rules.
    All four output sets (sel_features, ripper_rules, dt_rules, union) are
    computed here in a single pipeline execution.

    Returns dict with all fields needed by subsequent sections:
        case_data, declare_features, R_obs_all (dict of 4 R_obs values),
        sel_features, ripper_rules_orig, dt_rules_orig, union_rules_orig,
        fisher_scores, feat_names, m_total, positional_log, sequence_log,
        y, ordered_ids, config, wall_seconds.
    """
...
def collect_R_obs(dvm_out: dict) -> dict:
    """
    R_obs for each of the four output sets.

    sel_features  : #{features selected by Fisher+coverage} on real data.
    ripper_rules  : #{rules extracted by best-k RipperK} on real data.
    dt_rules      : #{rules extracted by best-depth DT} on real data.
    ripper+dt     : #{unique rules in ripper_rules u dt_rules} on real data.
    """
...
# =============================================================================
# SECTION 3 — DvM BISE 2025 DIAGNOSTICS
# =============================================================================

def run_diagnostics_dvm(dvm_out: dict) -> dict:
    """
    Report Fisher score distribution and coverage selection diagnostics.
    """
    print("\n" + "=" * 100)
    ...
    print(f"\n  M_all (declare features discovered) = {m:,}")
    print(f"  R_obs (sel_features)  = {R_obs_all['sel']:,}  "
          f"(coverage_threshold={cov_t})")
    print(f"  R_obs (ripper_rules)  = {R_obs_all['ripper']:,}")
    print(f"  R_obs (dt_rules)      = {R_obs_all['dt']:,}")
    print(f"  R_obs (ripper u dt)   = {R_obs_all['union']:,}")

    print(f"\n  Fisher score distribution over M_all ({m:,} features):")
    if len(finite_f) > 0:
        print(f"    mean   = {finite_f.mean():.4f}")
        print(f"    median = {np.median(finite_f):.4f}")
        print(f"    min    = {finite_f.min():.6f}")
        print(f"    max    = {finite_f.max():.4f}")
        print(f"    > 0  (discriminative):   {int((finite_f > 0).sum()):>6,}")
        print(f"    = 0  (symmetric / zero): {int((finite_f == 0).sum()):>6,}")
        print(f"    inf / non-finite:         {n_nonfinite:>6,}")
    else:
        print("    (no finite Fisher scores — degenerate encoding)")

    print(f"\n  No mechanistic prediction: Fisher score has no Type I error bound.")
    print(f"  E[V_b] is purely empirical (doubly-null protocol).")
...
def _rebuild_activity_index(trace: list) -> dict:
    """Build {activity: [sorted positions]} from an ordered activity list."""
    index = {}
...
def _build_doubly_nullified_log(
    ...
    """
    Build a doubly-nullified log: sigma_label composed with sigma_trace.

    sigma_label: replace each case's outcome with permuted_labels[i].
    sigma_trace: randomly permute activities within each trace.

    Shallow copies of CaseInfo are created; case_data_orig is never mutated.
    activity_index is rebuilt from the shuffled trace so positional_log
    derived from the null log correctly reflects the shuffled order.
    """
...
# =============================================================================
# SECTION 5 — DOUBLY-NULL REPLICATE RUNNER (DvM BISE 2025)
# =============================================================================

def _evaluate_null_dvm_all_sets(
    null_case_data: dict,
    declare_features: list,
    ...
    Returns V_b for all four output sets from a SINGLE evaluation:
        sel    — number of sel_features selected by Fisher+coverage
        ripper — number of rules extracted by RipperK on null sel submatrix
        dt     — number of rules extracted by DT on null sel submatrix
        union  — number of unique rules in ripper U dt on the null log

    CRITICAL: declare_features is the pool discovered from the ORIGINAL log.
    It is NOT re-discovered from the null log.
    """
...
def run_doubly_null_replicate_dvm(
    permuted_labels: np.ndarray,
    case_data_orig: dict,
    declare_features: list,
    case_ids_sorted: list,
    ...
    """
    Run DvM BISE 2025 on a single doubly-nullified replicate.

    Returns {'sel': int, 'ripper': int, 'dt': int, 'union': int} — V_b for
    all four output sets from a single null log evaluation.
    """
    ...
    with _suppress_output():
        counts = _evaluate_null_dvm_all_sets(
            ...
            declare_features   = declare_features,
            coverage_threshold = int(DVM_CONFIG.get("coverage_threshold",
                                                     DVM_COVERAGE_THRESHOLD)),
...
def _null_worker_dvm(
    ...
    """Joblib top-level worker for one DvM BISE 2025 doubly-null replicate (loky-safe)."""
...
def run_null_permutations(
    ...
    Each replicate returns V_b for all four output sets simultaneously
    (sel_features, ripper_rules, dt_rules, ripper+dt union) from a single
    null log evaluation — no re-running per set.

    Returns:
        dict with:
            null_counts  -- dict of {sel/ripper/dt/union: (B,) int array}
            wall_seconds -- float
    """
    print("\n" + "=" * 100)
    print("SECTION 6 — PARALLEL DOUBLY-NULL HELD-OUT PERMUTATIONS (DvM BISE 2025)")
    print(f"  B_null={B_NULL}, n_jobs={n_jobs}")
    print(f"  Fixed candidate pool: M_all declare features from ORIGINAL log")
    print(f"  ({len(declare_features):,} features frozen from original-data run)")
    print(f"\n  Per-replicate protocol (four output sets from single evaluation):")
    print(f"    1. sigma_trace: shuffle activities within each trace")
    print(f"    2. sigma_label: permute class labels (marginals preserved)")
    print(f"    3. Encode fixed M_all declare features on null log")
    print(f"    4. Fisher+coverage -> V_b[sel]")
    print(f"    5. RipperK on sel submatrix -> V_b[ripper]")
    print(f"    6. DT on sel submatrix -> V_b[dt]")
    print(f"    7. V_b[union] = |ripper_rules u dt_rules|")
    print(f"\n  CRITICAL: declare_features FIXED from original log — NOT re-discovered.")
    print("=" * 100)
    ...
    print(f"\n  Launching {B_NULL} parallel DvM workers (n_jobs={n_jobs})...\n")
    ...
    print(f"\n  V_b summary over {B_NULL} replicates:")
    for k in ('sel', 'ripper', 'dt', 'union'):
        arr = null_counts[k]
...
def compute_and_save_metrics(
    ...
    """
    Compute FDR_emp / PCER_emp / FWER_emp with BCa 95% CIs for all four sets.

    Saves results to two subfolders:
        sel_features/   — existing sel_features metrics + diagnostics
        rules/          — ripper_rules, dt_rules, ripper+dt metrics

    Returns: (results_by_set, fdr_tests)
        results_by_set : dict mapping set name -> DataFrame row
        fdr_tests      : dict with pass/fail verdict per set
    """
    print("\n" + "=" * 100)
    ...
    print(f"\n  FDR Results (Sepsis, B_null={B_NULL}, doubly-null protocol):")
    hdr = (f"  {'Set':18s} {'alpha':>5s} {'R_obs':>6s} {'E[V_b]':>8s} "
    ...
    for k, label in SET_LABELS.items():
        ...
        fdr_tests[k]   = {
            'fdr_emp':      m['FDR_emp'],
            'controls_FDR': m['controls_FDR'],
            'FDR_CI_lower': m['FDR_CI_lower'],
            'FDR_CI_upper': m['FDR_CI_upper'],
        }
        ci_str  = f"[{m['FDR_CI_lower']:.4f}, {m['FDR_CI_upper']:.4f}]"
        verdict = "PASS" if m['controls_FDR'] else "FAIL"
        print(
            f"  {label:18s} {ALPHA:>5.3f} {m['R_obs']:>6d} "
    ...
    sel_df = pd.DataFrame([{'method': METHOD_DM, 'output_set': 'sel_features',
                             **m_sel}])
    ...
    # Diagnostics CSV (Fisher scores — sel_features specific)
    declare_features = dvm_out["declare_features"]
    fisher_scores    = dvm_out["fisher_scores"]
    feat_names       = dvm_out["feat_names"]
    ...
    # ── rules subfolder ───────────────────────────────────────────────────
    RULE_SETS = {
        'ripper': ('ripper_rules', 'ripper_rules_orig'),
        'dt':     ('dt_rules',     'dt_rules_orig'),
        'union':  ('ripper+dt_rules', 'union_rules_orig'),
    }
    SUFFIX = {'ripper': 'ripper', 'dt': 'dt', 'union': 'union'}

    for k, (set_label, orig_key) in RULE_SETS.items():
        m_k = all_metrics[k]
        suf = SUFFIX[k]

        row_df = pd.DataFrame([{'method': METHOD_DM, 'output_set': set_label, **m_k}])
        path   = os.path.join(rules_dir, f"rq1_dvm_bise2025_fdr_metrics_{suf}.csv")
        row_df.to_csv(path, index=False)
        print(f"  Saved: {path}")

        nc_df            = pd.DataFrame({'V_b': null_counts[k]})
        nc_df.index.name = 'replicate_b'
        path = os.path.join(rules_dir, f"rq1_dvm_bise2025_null_counts_{suf}.csv")
        nc_df.to_csv(path)
        print(f"  Saved: {path}")
    ...
    rules_json = {
        ...
        'config': {
            'B_NULL': B_NULL, 'ALPHA': ALPHA, 'BASE_SEED': BASE_SEED,
            'N_JOBS': N_JOBS, 'coverage_threshold': cov_t,
            'RIPPER_K_GRID':  list(RIPPER_K_GRID),
            'DT_DEPTH_GRID':  list(DT_DEPTH_GRID),
        },
        'R_obs_all': R_obs,
        ...
        'null_replicate_summary': {
            k: {
                'mean_V_b':   float(null_counts[k].mean()),
                'std_V_b':    float(null_counts[k].std()),
                'max_V_b':    int(null_counts[k].max()),
                'n_zero_V_b': int(np.sum(null_counts[k] == 0)),
            }
...
def main():
    print("\n" + "=" * 100)
    print("RQ1 — FDR CONTROL VALIDITY: SEPSIS  [DvM BISE 2025]")
    ...
    print(f"  support_threshold  = {DVM_CONFIG.get('support_threshold', DVM_SUPPORT_THRESHOLD)}")
    print(f"  alpha (nominal FDR) = {ALPHA}  (for pass/fail verdict only)")
    ...
    print(f"\n{'='*100}")
    print("RQ1 — SEPSIS DvM BISE 2025 COMPLETE  (double-null protocol)")
    print(f"{'='*100}")
    print(f"  Total wall time: {total_wall:.1f}s ({total_wall/3600:.2f} h)")
    print(f"  Output: {RQ1_OUTPUT_DIR}/sel_features/  and  {RQ1_OUTPUT_DIR}/rules/")
    print(f"\n  KEY RESULTS:")
    print(f"  {'Output set':18s} {'alpha_nom':>9s} {'R_obs':>6s} "
          f"{'E[V_b]':>8s} {'FDR_emp':>8s} {'Pass?':>6s}")
    print(f"  {'─'*60}")
    for k, label in SET_LABELS.items():
        m   = all_metrics[k]
        fdr = m['FDR_emp']
        ...
        print(f"  {label:18s} {ALPHA:>9.3f} {R_obs[k]:>6d} "
              f"{ev:>8.2f} {fdr:>8.4f} {verdict:>6s}")

    print(f"\n  FDR_emp measured empirically via doubly-null protocol.")
    print(f"  No mechanistic prediction — Fisher score has no Type I error bound.")
    print(f"{'='*100}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            "RQ1 FDR Validity — Sepsis  "
            "(DvM BISE 2025 Fisher+coverage+rules baseline, double-null protocol)"
        )
    )
    parser.add_argument(
