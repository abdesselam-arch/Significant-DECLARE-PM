"""
drva_BPI_17_parallel.py  —  DRVA Baseline: BPI Challenge 2017 Event Log
=========================================================================
...
(Algorithm 1, Cecconi et al. 2021) on the BPI Challenge 2017 event log, using the
...
def run_drva(
    ...
    print(f"   L_B (Accepted (Normal), class 0): {n_B:,}")
...
def save_outputs(drva_out: dict) -> None:
    ...
    full_json = {
        ...
        'scientific_description': {
            'null_hypothesis': (
                'No difference in Confidence between Deviant (Not-Accepted) '
                'and Normal (Accepted) variants for rule r.'
            ),
            'permutation_scheme': (
    ...
    lines.append(f"  Negative (Accepted-dominant):  {n_neg:,}")
    lines.append("")
    lines.append("=" * 120)
...
def main():
    parser = argparse.ArgumentParser(
        description="DRVA Baseline — BPI_17  (shared M_all design, v2.1)"
    )
