"""
drva_Sepsis_parallel.py  —  DRVA Baseline: Sepsis ICU Event Log
================================================================
...
(Algorithm 1, Cecconi et al. 2021) on the Sepsis ICU event log, using the
...
OUTPUT_DIR        = "DRVA_Sepsis"
os.makedirs(OUTPUT_DIR, exist_ok=True)

DRVA_CONFIG = {
    'pi':                   2000,   # permutation iterations (π)
    'alpha':                0.05,   # per-rule significance level α
...
def run_drva(
    ...
    print(f"   L_B (Normal,  class 0): {n_B:,}")
...
def save_outputs(drva_out: dict) -> None:
    ...
    full_json = {
        ...
        'scientific_description': {
            'null_hypothesis': (
                'No difference in Confidence between Deviant (Return ER) '
                'and Normal (No Return ER) variants for rule r.'
            ),
            'permutation_scheme': (
...
def run_drva_on_doubly_null_log(
    ...
    Args:
        ...
        holds_all:       Optional precomputed holds dict {spec: {case_id: 0/1}}.
                         When provided, skips evaluate_pattern_fast re-evaluation
                         and builds the encoding matrix directly from holds_all.
                         This is the fast path for RQ1 null replicates where
                         compute_holds_by_case_batch has already been called.
                         tf_null_matrix (built under the real data distribution)
                         is intentionally NOT passed here; it would create a
                         distributional mismatch that inflates FDR_emp.
    ...
    if holds_all is not None:
        # Fast path: convert precomputed holds dict to the (n, m) encoding matrix,
        # avoiding evaluate_pattern_fast re-evaluation (dominant cost per replicate).
...
def main():
    parser = argparse.ArgumentParser(
        description="DRVA Baseline — Sepsis ICU  (shared M_all design, v2.0)"
    )
    ...
    print("\n" + "=" * 100)
    print("DRVA — DECLARATIVE RULES VARIANT ANALYSIS  (shared M_all design, v2.0)")
