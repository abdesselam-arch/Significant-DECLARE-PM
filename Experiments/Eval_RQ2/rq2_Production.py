...
===============================================
Full Evaluation Pipeline: Steps 1–6  (Production)

STEP 1: Retrieve holds_all from Phase 1 output — reuse if in-memory or
        cached to disk; recompute only as last resort.
        NOTE: Production uses a pre-existing case label attribute ('regular'/'deviant').
        No outcome signal is stripped from traces — full sequence retained.
...
Version: 4.0-PRODUCTION-P1-ALIGNED
...
- van Dongen (2012). BPI Challenge 2012 Production Process. 4TU.ResearchData.
"""
...
# ── Log configuration ────────────────────────────────────────────────────────
# outcome_mode='label_attribute': class 1 iff label column value != label_normal_value.
# Production uses a pre-existing case attribute ('regular'/'deviant') — no activity
# stripping needed. Full trace is retained. CSV sep=';' (semicolon-delimited export).
LOG_CONFIGS = {
    'Production': {
        'csv': 'Production.csv',
        'declare_spec': 'phase0_Production.json',
        'phase1_dir': 'Production_Results',
        'phase1_json': 'Production_Results/three_hypothesis_houfdr_results.json',
        'outcome_signal': None,             # no activity-based outcome signal
        'outcome_mode': 'label_attribute',  # label read from pre-existing case column
        'label_col': 'label',               # column holding 'regular' / 'deviant'
        'label_normal_value': 'regular',    # value mapped to Class 0 (Normal)
        'outcome_strip_from_trace': False,  # full trace retained — no stripping
        'csv_sep': ';',                     # semicolon-delimited (ProM export)
        'case_col': 'Case ID',
        'act_col': 'Activity',
...
RQ2_OUTPUT_DIR = 'RQ2_Production_Parallel'

# LOG_CONFIGS = {
#     'Production': {
#         'csv': '../Phase 1 - KM Catalog Construction/Experiments data/CSV/Production.csv',
#         'declare_spec': '../Phase 1 - KM Catalog Construction/Experiments data/Experiments/Results/DECspec_Production/phase0_Production.json',
#         'phase1_dir': '../Phase 1 - KM Catalog Construction/Experiments data/Experiments/Results/Production_Results',
#         'phase1_json': '../Phase 1 - KM Catalog Construction/Experiments data/Experiments/Results/Production_Results/three_hypothesis_houfdr_results.json',
#         'outcome_signal': None,
#         'outcome_mode': 'label_attribute',
#         'label_col': 'label',
#         'label_normal_value': 'regular',
#         'outcome_strip_from_trace': False,
#         'csv_sep': ';',
#         'case_col': 'Case ID',
#         'act_col': 'Activity',
#         'ts_col': 'Complete Timestamp',
...
# # RQ2 output directory
# RQ2_OUTPUT_DIR = '../Experiments data/Experiments/Results/RQ2_Production'
...
# evaluate_pattern_fast dispatch (identical to p1_Production_hou.py) but are NOT
# listed in ALL_CONSTRAINT_TYPES — p1_Production_hou.py generates candidates from
...
# ── Precedence-family and Not-Precedence checkers (identical to p1_Production_hou.py)
...
class CaseInfo:
    ...
    IMPORTANT — `trace` retains the FULL activity sequence. The Production
    outcome is determined from the pre-existing 'label' case attribute
    ('regular' = Class 0, 'deviant' = Class 1), not from any activity in the
    trace, so no outcome-signal stripping is needed or applied. This matches
    P1's load_and_preprocess_data() which reads the label column directly
    (see outcome_mode='label_attribute' in LOG_CONFIGS).
    """
    case_id: str
    outcome: int
    trace: List[str]
...
def load_event_log(
    ...
    PRODUCTION NOTE: outcome_mode='label_attribute'. The outcome is read from
    the pre-existing 'label' case column ('regular' → Class 0, 'deviant' → Class 1).
    This matches Phase 1 (p1_Production_hou.py) preprocessing exactly:

        Phase 1:  outcome = 0 if label_raw == 'regular' else 1
        RQ2:      outcome = 0 if str(label_raw).strip().lower() == label_normal_value else 1

    No trace stripping is performed. Full activity sequence is used for pattern
    evaluation, matching Phase 1 which retains the complete trace as-is.
    CSV separator is ';' (semicolon-delimited ProM export).
    """
    print(f"\n   Loading event log: {csv_path}")

    csv_sep  = log_config.get('csv_sep', ',')
    ...
    label_col           = log_config.get('label_col')
    label_normal_value  = log_config.get('label_normal_value', 'regular')
    ...
    if outcome_mode == 'label_attribute':
        assert label_col in df.columns, f"Label column '{label_col}' not found. Columns: {list(df.columns)}"

    print(f"   Columns: case={case_col}, act={act_col}, ts={ts_col}")
    print(f"   Events: {len(df):,}, Cases: {df[case_col].nunique():,}")
    print(f"   Outcome mode: {outcome_mode}  (label_col='{label_col}', normal='{label_normal_value}')")
    print(f"   Strip outcome from trace: False (label_attribute mode — full trace retained)")
    ...
    for case_id, group in df.groupby(case_col):
        ...
        # Determine outcome from pre-existing label attribute (matches Phase 1)
        if outcome_mode == 'label_attribute':
            label_raw = case_events[label_col].iloc[0]
            if pd.isna(label_raw):
                n_skipped_label += 1
                continue
            outcome = 0 if str(label_raw).strip().lower() == label_normal_value else 1
            trace = raw_trace  # full sequence; no stripping needed
        else:
    ...
    if n_skipped_label > 0:
        print(f"   ⚠️  Skipped {n_skipped_label} cases with missing label attribute")

    assert n1 > 0, (
        f"FATAL: All {len(case_data)} cases have outcome=0. "
        f"Check LOG_CONFIGS['{log_name}']['label_col'] and 'label_normal_value'."
    )
    assert n0 > 0, (
    ...
    print(f"   Class 0 (Regular / Normal):                 {n0:,} ({n0/len(case_data)*100:.1f}%)")
...
#   "Positive" = class 1 (Deviant / re-submission required) has higher prevalence (prev1 ≥ prev0)
#   "Negative" = class 0 (Regular / Normal) has higher prevalence (prev0 > prev1)
# ============================================================================
...
def generate_rq2_outputs(
    ...
    rpt.append(f"Version: 4.0-PRODUCTION-P1-ALIGNED")
    rpt.append("=" * 100)
    ...
    rpt.append("DATASET")
    rpt.append("─" * 100)
    rpt.append(f"  n = {len(y):,}  (Class 1 Deviant: {n1:,}, Class 0 Regular/Normal: {n0:,})")
    rpt.append(f"  Class ratio: {n1/len(y)*100:.1f}% / {n0/len(y)*100:.1f}%")
    rpt.append(f"  Outcome mode: label_attribute ('regular'→0, 'deviant'→1); full trace retained")
    rpt.append("")
...
def main():
    ...
    print(f"   Outcome mode: label_attribute (label col → 'regular'=0, 'deviant'=1; full trace retained)")
