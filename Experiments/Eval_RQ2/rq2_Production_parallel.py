...
STEP 1: Retrieve holds_all from Phase 1 output — reuse if in-memory or
        ...
        No outcome-signal stripping: the Production label is a dedicated
        attribute column, not an activity event in the trace.
...
Version: 3.0-PRODUCTION-DIRECTION-AWARE
...
# ── Log configuration ────────────────────────────────────────────────────────
# outcome_mode='label_column': class 1 iff label column == 'deviant'.
# outcome_strip_from_trace: False — the Production label is a dedicated
#   attribute column and does not appear as an activity event in the trace.
#   All activity events carry genuine process-behavioural content; no stripping
#   needed (and no signal to strip). This matches p1_Production_parallel.py.
# sep: semicolon-delimited CSV.
LOG_CONFIGS = {
    'Production': {
        'csv': 'Production.csv',
        'declare_spec': 'phase0_Production.json',
        'phase1_dir': 'Production_ThreeHyp_SAM_Parallel',
        'phase1_json': 'Production_ThreeHyp_SAM_Parallel/three_hypothesis_samfdr_results.json',
        'outcome_signal': 'deviant',          # value in 'label' column
        'outcome_mode': 'label_column',       # read from dedicated label col
        'outcome_strip_from_trace': False,    # no signal to strip
        'case_col': 'Case ID',
        'act_col': 'Activity',
...
#         'declare_spec': '../Phase 1 - KM Catalog Construction/Experiments data/Experiments/Results/DECspec_Production/phase0_declare_specification_CC.json',
#         'phase1_dir': '../Phase 1 - KM Catalog Construction/Experiments data/Experiments/Results/Production_ThreeHyp_SAM_Parallel',
#         'phase1_json': '../Phase 1 - KM Catalog Construction/Experiments data/Experiments/Results/Production_ThreeHyp_SAM_Parallel/three_hypothesis_samfdr_results.json',
#         'outcome_signal': 'deviant',
#         'outcome_mode': 'label_column',
...
def load_event_log(
    ...
    Supports two outcome modes controlled by LOG_CONFIGS:

    'activity_present' (e.g. Sepsis):
        Class 1 iff outcome_signal ∈ trace activities.
        outcome_strip_from_trace=True removes the signal activity from traces
        before pattern evaluation to match Phase 1 preprocessing exactly.

    'label_column' (e.g. Production):
        Class 1 iff label_col value == outcome_signal ('deviant').
        outcome_strip_from_trace=False — the label is a dedicated attribute
        column that never appears as a trace activity; no stripping needed.
        CSV is read with sep=log_config.get('sep', ',').
    """
    print(f"\n   Loading event log: {csv_path}")

    sep = log_config.get('sep', ',')
    ...
    label_col      = log_config.get('label_col', None)
    ...
    if outcome_mode == 'label_column':
        assert label_col and label_col in df.columns, \
            f"Column '{label_col}' not found. Columns: {list(df.columns)}"

    print(f"   Columns: case={case_col}, act={act_col}, ts={ts_col}")
    print(f"   Events: {len(df):,}, Cases: {df[case_col].nunique():,}")
    print(f"   Outcome mode: {outcome_mode} (signal: '{outcome_signal}')")
    ...
    for case_id, group in df.groupby(case_col):
        ...
        raw_trace = case_events[act_col].dropna().tolist()
        ...
        elif outcome_mode == 'label_column':
            # Label is case-level: read from first non-null entry
            raw_label = case_events[label_col].dropna().iloc[0] \
                if case_events[label_col].notna().any() else ''
            outcome = 1 if str(raw_label).strip() == outcome_signal else 0
        else:
            raise ValueError(f"Unknown outcome_mode: '{outcome_mode}'.")

        # Strip outcome signal from trace if required (not needed for Production)
        if strip_outcome:
...
def generate_rq2_outputs(
    ...
    rpt.append(f"Version: 3.0-PRODUCTION-DIRECTION-AWARE")
    rpt.append("=" * 100)
    ...
    rpt.append("DATASET")
    rpt.append("─" * 100)
    rpt.append(f"  n = {len(y):,}  (Class 1 Deviant: {n1:,}, Class 0 Regular: {n0:,})")
    rpt.append(f"  Class ratio: {n1/len(y)*100:.1f}% / {n0/len(y)*100:.1f}%")
    rpt.append(f"  Outcome label from dedicated 'label' column (no trace stripping)")
    rpt.append("")
...
def main():
    ...
    print("Version 3.0 — PRODUCTION — DIRECTION-AWARE")
    print("=" * 100)
    print(f"\n🎯 EXPERIMENTAL DESIGN:")
    ...
    print(f"   Outcome: pre-encoded 'label' column (no trace stripping — Production)")
