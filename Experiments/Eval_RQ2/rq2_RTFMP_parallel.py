...
STEP 1: Retrieve holds_all from Phase 1 output — reuse if in-memory or
        ...
        CRITICAL: two preprocessing steps must exactly match Phase 1:
        (1) lifecycle filter — only COMPLETE events retained;
        (2) outcome signal ('Send for Credit Collection') stripped from
...
LOG_CONFIGS = {
    'RTFMP': {
        ...
        'phase1_dir': 'RTFMP_ThreeHyp_SAM_Parallel',
        'phase1_json': 'RTFMP_ThreeHyp_SAM_Parallel/three_hypothesis_samfdr_results.json',
...
# LOG_CONFIGS = {
#     'RTFMP': {
#         'csv': f'{BASE_DATA}/CSV/Road_Traffic_Fine_Management_Process.csv',
#         'declare_spec': f'{BASE_DATA}/Experiments/Results/DECspec_RTFMP/phase0_declare_specification_CC.json',
#         'phase1_dir': f'{BASE_DATA}/Experiments/Results/RTFMP_ThreeHyp_SAM_Parallel',
#         'phase1_json': f'{BASE_DATA}/Experiments/Results/RTFMP_ThreeHyp_SAM_Parallel/three_hypothesis_samfdr_results.json',
#         'outcome_signal': 'Send for Credit Collection',
#         'outcome_mode': 'activity_present',
...
# RQ2_OUTPUT_DIR = f'{BASE_DATA}/Experiments/Results/RQ2_RTFMP_Parallel'
...
def generate_rq2_outputs(
    ...
    rpt.append("DATASET")
    rpt.append("─" * 100)
    rpt.append(f"  n = {len(y):,}  (Class 1 Credit Collection: {n1:,}, Class 0 No Credit Collection: {n0:,})")
    rpt.append(f"  Class ratio: {n1/len(y)*100:.1f}% / {n0/len(y)*100:.1f}%")
    rpt.append(f"  Lifecycle filter: lifecycle:transition == 'complete' (matching Phase 1)")
    rpt.append(f"  Outcome signal stripped from traces: 'Send for Credit Collection' (matching Phase 1)")
    rpt.append("")
...
def main():
    ...
    print(f"   Lifecycle filter: lifecycle:transition == 'complete' (Phase 1 consistency)")
