...
===============================================
Full Evaluation Pipeline: Steps 1–6  (BPI Challenge 2015)
...
- van Dongen et al. (2015). BPI Challenge 2015. 4TU.ResearchData.
"""
...
# outcome_mode='case_attribute_bpi15': BPI-2015 case-level attribute labelling.
#   Pre-filters to lifecycle:transition == 'COMPLETE', then reads case:caseStatus
#   per case ('G' → 0 Normal/Granted, else → 1 Deviant/Not-Granted).
#   No outcome signal stripping — label is a case attribute, not an activity.
#   Matches p1_BPI_15.py preprocessing exactly.
LOG_CONFIGS = {
    'BPI15': {
        'csv': '../Experiments data/CSV/BPI_Challenge_2015_1.csv',
        'declare_spec': '../Experiments data/Experiments/Results/DECspec_BPI15/phase0_declare_specification_CC.json',
        'phase1_dir': '../Experiments data/Experiments/Results/BPI15_ThreeHyp_SAM',
        'phase1_json': '../Experiments data/Experiments/Results/BPI15_ThreeHyp_SAM/three_hypothesis_samfdr_results.json',
        # BPI-2015 case-attribute labelling
        # Class 0 (Normal):  case:caseStatus == 'G'  (Granted)
        # Class 1 (Deviant): case:caseStatus != 'G'  (Not-Granted)
        'outcome_mode': 'case_attribute_bpi15',
        'outcome_strip_from_trace': False,  # label is a case attribute — nothing to strip
        'lifecycle_filter': 'complete',     # retain only lifecycle:transition == 'COMPLETE'
        'case_col': 'case:concept:name',
        'act_col': 'concept:name',
        'ts_col': 'time:timestamp',
        'lifecycle_col': 'lifecycle:transition',
        'label_col': 'case:caseStatus',
        'normal_label_value': 'G',          # 'G' → class 0 (Granted / Normal)
    },
}

# RQ2 output directory
RQ2_OUTPUT_DIR = '../Experiments data/Experiments/Results/RQ2_BPI15'
...
def load_event_log(
    ...
    CRITICAL FOR SEPSIS: When outcome_strip_from_trace is True, the outcome
    signal activity is stripped from traces AFTER labelling but BEFORE pattern
    evaluation. This matches Phase 1 (p1_sepsis.py) preprocessing exactly:

        Phase 1:  trace = [a for a in all_activities if a != RETURN_ER]
    ...
    'Return ER' would have different satisfaction values).

    CRITICAL FOR BPI-2015 (outcome_mode='case_attribute_bpi15'):
    Replicates p1_BPI_15.py preprocessing exactly:
        1. Pre-filter to lifecycle:transition == 'COMPLETE'.
        2. Read case:caseStatus attribute per case.
           'G' → class 0 (Normal/Granted); anything else → class 1 (Deviant/Not-Granted).
        3. Trace = all COMPLETE activities (no stripping — label is a case attribute).
    """
    ...
    print(f"   Events (raw): {len(df):,}, Cases: {df[case_col].nunique():,}")
    print(f"   Outcome mode: {outcome_mode}")

    # ── BPI-2015 case-attribute labelling ────────────────────────────────────
    if outcome_mode == 'case_attribute_bpi15':
        lifecycle_col      = log_config.get('lifecycle_col', 'lifecycle:transition')
        label_col          = log_config.get('label_col', 'case:caseStatus')
        normal_label_value = log_config.get('normal_label_value', 'G')

        assert lifecycle_col in df.columns, (
            f"Column '{lifecycle_col}' not found. Required for outcome_mode='case_attribute_bpi15'."
        )
        assert label_col in df.columns, (
            f"Column '{label_col}' not found. Required for outcome_mode='case_attribute_bpi15'."
        )

        # Step 1: retain only COMPLETE lifecycle events (matches p1_BPI_15.py)
        df = df[df[lifecycle_col].str.upper() == 'COMPLETE'].copy()
        print(f"   After COMPLETE filter: {len(df):,} events, "
              f"{df[case_col].nunique():,} cases")
        print(f"   Label column: '{label_col}' — '{normal_label_value}' → 0 (Normal), else → 1 (Deviant)")

        # Step 2: build case-level status lookup from first non-null value per case
        case_status = (
            df.dropna(subset=[label_col])
              .groupby(case_col)[label_col]
              .first()
              .to_dict()
        )

        case_data = {}
        skipped = {'empty_trace': 0, 'missing_label': 0}

        for case_id, group in df.groupby(case_col):
            case_events = group.sort_values(ts_col)
            trace = case_events[act_col].tolist()

            if len(trace) == 0:
                skipped['empty_trace'] += 1
                continue

            # Step 3: outcome from case-level attribute (no stripping needed)
            status = case_status.get(case_id, None)
            if status is None:
                skipped['missing_label'] += 1
                outcome = 1   # conservative: treat missing status as Deviant
            else:
                outcome = 0 if status == normal_label_value else 1
        ...
        assert n1 > 0, (
            ...
            f"Check LOG_CONFIGS['{log_name}']['normal_label_value']."
        )
        assert n0 > 0, (
            f"FATAL: All {len(case_data)} cases have outcome=1. "
            f"Check LOG_CONFIGS['{log_name}']['label_col']."
        )
    ...
    # ── Standard activity-present labelling ──────────────────────────────────
    outcome_signal = log_config['outcome_signal']
    print(f"   Outcome signal: '{outcome_signal}'")
    ...
    for case_id, group in df.groupby(case_col):
        ...
        # Strip outcome signal from trace (CRITICAL for Sepsis Phase 1 consistency)
        if strip_outcome:
    ...
    print(f"   Class 1 (Deviant):  {n1:,} ({n1/len(case_data)*100:.1f}%)")
    print(f"   Class 0 (Normal):   {n0:,} ({n0/len(case_data)*100:.1f}%)")
...
def main():
    ...
    print("Version 3.0 — BPI CHALLENGE 2015 — DIRECTION-AWARE")
    print("=" * 100)
    print(f"\n🎯 EXPERIMENTAL DESIGN:")
    print(f"   Log: BPI Challenge 2015 (case-attribute labelling: case:caseStatus 'G' vs. not-'G')")
    ...
    print(f"   Outcome stripping: none (label is case attribute, not an activity)")
