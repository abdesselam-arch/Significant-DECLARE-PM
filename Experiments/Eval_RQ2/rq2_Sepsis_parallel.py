#!/usr/bin/env python3
"""
rq2_Sepsis_parallel.py  —  RQ2 Specification Quality Degradation Analysis
==========================================================================
...
from BaselinesRQ1.DeclareMiner_Sepsis import (
    run_declareminer,
...
# B_null per level
# Sepsis m_prime ≈ 245: theoretical minimum B_null ≈ 3919; cap at 200 (full P1 budget)
...
def compute_minimum_b_null(
    ...
    """
    Minimum B_null for the Storey q-value gate to have any power.

    From q_(1) = π̂₀ × m' × p̃_min / 1 ≤ α and p̃_min = 1/(B_null+1):
        B_null_min = ⌈π̂₀ × m' / α⌉ - 1

    The safety multiplier accounts for stochasticity of T_Hou^(b) under the
    null inflating above T_Hou_obs. Result clamped to [50, 500].
    """
    if m_prime == 0:
        return 50
    b_theoretical = int(np.ceil(pi0_hat * m_prime / alpha)) - 1
    b_recommended = int(np.ceil(b_theoretical * safety))
    return max(50, min(500, b_recommended))
...
def analyze_level(
    ...
    # ── Adaptive B_null + skip null replicates when R_obs=0 ──────────────
    ...
    # Adaptive budget: compute theoretical minimum, cap at B_NULL_ANCHOR
    B_null_adaptive = compute_minimum_b_null(m_prime_p1, alpha)
    B_null_this = min(B_null_adaptive, B_NULL_ANCHOR) if is_anchor else B_NULL_INTERMEDIATE

    methods_need_null = [mth for mth in ALL_METHODS if R_obs[mth] > 0]
    methods_zero_r    = [mth for mth in ALL_METHODS if R_obs[mth] == 0]

    # Null counts initialised to empty; populated only when replicates run
    null_counts = {mth: np.zeros(0, dtype=int) for mth in ALL_METHODS}
    fdr_metrics = {}

    # FDR_emp = E[V_b]/R_obs is undefined when R_obs=0; skip and report NaN
    for mth in methods_zero_r:
        fdr_metrics[mth] = _null_fdr_skipped('R_obs=0: FDR_emp undefined')

    # Null replicates only at anchor levels and only when ≥1 method has R_obs>0
    if is_anchor and methods_need_null:
        print(
            f"  [{operator}  ℓ={level:.2f}]  "
            f"Running {B_null_this} null replicates "
            f"(m_prime={m_prime_p1}, B_null_adaptive={B_null_adaptive}, "
            f"{len(methods_need_null)} methods with R_obs>0)...", flush=True
        )
        ...
        for mth in methods_need_null:
            fdr_metrics[mth] = _compute_fdr_from_null(
                null_counts[mth], R_obs[mth], alpha_vals[mth]
            )
    else:
        reason = 'non-anchor level' if not is_anchor else 'all methods R_obs=0'
        for mth in methods_need_null:
            fdr_metrics[mth] = _null_fdr_skipped(reason)
    ...
    return {
        ...
        'B_null':          B_null_this,
        'B_null_adaptive': B_null_adaptive,
        'wall_seconds':    wall,
...
def run_original_data(n_workers: int = 1) -> dict:
    """
    Run all three methods on the original (unperturbed) Sepsis log.
    ...
    for res in level_results:
        ...
        for mth in ALL_METHODS:
            ...
            rows.append({
                ...
                'estimable':      fdr['estimable'],
                'skipped_reason': fdr['skipped_reason'],
                'B_null':         res['B_null'],
                'B_null_adaptive': res.get('B_null_adaptive', float('nan')),
...
def _print_degradation_table(level_results: list, operator_label: str) -> None:
    ...
    for res in level_results:
        ...
        for mth in ALL_METHODS:
            ...
            fdr = res['fdr_metrics'][mth]
            if fdr['estimable']:
                fdr_str = f"{fdr['FDR_emp']:>8.4f}"
                ci_str  = f"[{fdr['FDR_CI_lower']:.3f},{fdr['FDR_CI_upper']:.3f}]"
                vrd     = "PASS" if fdr['controls_FDR'] else "FAIL"
            else:
                fdr_str = f"{'NaN':>8s}"
                ci_str  = f"{'[  NaN  ]':>22s}"
                reason  = fdr.get('skipped_reason', '')
                vrd     = f"{'N/A':>6s}"
                if res['R_obs'][mth] == 0:
                    vrd = " R=0"
            print(
                ...
                f"{fdr_str:>8s}  {ci_str:>22s}  {vrd:>6s}"
            )
    print(f"  {'─'*110}")
...
def main():
    print("\n" + "=" * 100)
    print("RQ2 — SPECIFICATION QUALITY DEGRADATION: SEPSIS ICU")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            "RQ2 Specification Degradation — Sepsis ICU  "
