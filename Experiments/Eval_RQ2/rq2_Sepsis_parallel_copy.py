...
RQ2_OUTPUT_DIR = "RQ2_Sepsis_Joint"
...
# Real-data P1 run budget — FAITHFULLY MATCHING P1 v9.0 (p1_Sepsis_hou.py)
...
# Set to -1 for standalone SLURM jobs (one (operator, level) per array task).
INNER_N_JOBS = 1
...
#       B_null_min = ⌈π̂₀ · m' / α⌉ − 1
#   Sepsis (m'≈245, π̂₀=0.8, α=0.05): min ≈ 3919 — infeasible per level.
...
def run_original_data(n_workers: int = 1) -> dict:
    """
    Run all three methods on the original (unperturbed) Sepsis ICU log.
...
def main():
    print("\n" + "=" * 100)
    print("RQ2 — SPECIFICATION QUALITY DEGRADATION: SEPSIS  (v3.0)")
    ...
    # Section 4: summary
    print(f"\n{'='*100}")
    print("RQ2 — SEPSIS COMPLETE  (v3.0 Joint Noise)")
    print(f"{'='*100}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            "RQ2 Specification Degradation — Sepsis v3.0  "
    ...
    parser.add_argument(
        ...
        help='Smoke test: 3×3 grid only, B_null=2, minimal budgets',
    )
    args = parser.parse_args()

    if args.dry_run:
        JOINT_LABEL_LEVELS  = [0.0, 0.10, 0.50]
        JOINT_STRUCT_LEVELS = [0.0, 0.30, 1.00]
        JOINT_GRID          = [(e, r) for e in JOINT_LABEL_LEVELS for r in JOINT_STRUCT_LEVELS]
        ANCHOR_CELLS        = frozenset({
            (0.0, 0.0),
            (JOINT_LABEL_LEVELS[-1], 0.0),
            (0.0, JOINT_STRUCT_LEVELS[-1]),
            (JOINT_LABEL_LEVELS[-1], JOINT_STRUCT_LEVELS[-1]),
        })
        LABEL_NOISE_LEVELS  = JOINT_LABEL_LEVELS
        STRUCT_NOISE_LEVELS = JOINT_STRUCT_LEVELS
        ...
        _C_NULL, _F_NULL = hou_satterthwaite_params(W_STRUCT_RQ2, W_DISC_RQ2, rho_sd=0.0)
        print("*** DRY-RUN MODE: 3×3 grid, B_null=2, minimal budgets ***")
    else:
