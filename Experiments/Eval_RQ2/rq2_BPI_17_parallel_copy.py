...
--------------
  ...
  S_orig_rq2    — RQ2 empirical Phipson-Smyth baseline at (ε=0, ρ=0).
                  Single baseline for all 49 cells.
...
HEATMAP OUTPUT
---------------
Primary deliverable: 7×7 FDR_ref heatmap per method.
Robustness score: Area{(ε,ρ) : FDR_ref(ε,ρ) ≤ 0.05}

WEIGHT FAITHFULNESS
-------------------
P1 (full): W_DISC = 1500/2500 = 0.60   W_STRUCT = 1000/2500 = 0.40
RQ2 real:  same — budgets identical to P1 v9.0.

OUTPUT FILES
-------------
    rq2_joint_metrics.csv, rq2_joint_<metric>_<method>.csv (heatmaps),
    rq2_joint_cross_jaccard.csv, rq2_joint_null_counts.json,
    rq2_joint_results.json
...
RQ2_OUTPUT_DIR = "RQ2_BPI_17_Joint"
...
# Real-data P1 run budget — FAITHFULLY MATCHING P1 v9.0 (p1_BPI_17_hou.py)
...
# BPI-17 m_prime may be large; cap at 200 (full P1 budget) for adequate power
...
def apply_joint_perturbation(
    ...
    """Joint operator N_label(ε) ∘ N_struct(ρ). struct_log has ρ pre-applied."""
...
#       B_null_min = ⌈π̂₀ · m' / α⌉ − 1
#   BPI_17 (m'≈195, π̂₀=0.8, α=0.05): min ≈ 3119 — infeasible per level.
#   → B_NULL_REAL=200 is used instead (matches full P1 budget; resolution 1/201 ≪ α).
...
def run_original_data(n_workers: int = 1) -> dict:
    """
    Run all three methods on the original (unperturbed) BPI Challenge 2017 log.
...
def main():
    print("\n" + "=" * 100)
    print("RQ2 — SPECIFICATION QUALITY DEGRADATION: BPI-17  (v3.0)")
    ...
    # Section 4: summary
    print(f"\n{'='*100}")
    print("RQ2 — BPI-17 COMPLETE  (v3.0 Joint Noise)")
    print(f"{'='*100}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            "RQ2 Specification Degradation — BPI-17 v3.0  "
    ...
    if args.dry_run:
        ...
        JOINT_LABEL_LEVELS  = LABEL_NOISE_LEVELS
        JOINT_STRUCT_LEVELS = STRUCT_NOISE_LEVELS
        JOINT_GRID = [
            (eps, rho)
            for eps in JOINT_LABEL_LEVELS
            for rho in JOINT_STRUCT_LEVELS
