#!/usr/bin/env python3
"""
rq2_RTFMP_parallel.py  —  RQ2 Specification Quality Degradation Analysis
==========================================================================
...
# ── DRVA baseline ─────────────────────────────────────────────────────────
from BaselinesRQ1.DRVA_RTFMP import (
    run_drva,
...
from BaselinesRQ1.DeclareMiner_RTFMP import (
    run_declareminer,
...
RQ2_OUTPUT_DIR = "RQ2_RTFMP_Joint"
...
# Real-data P1 run budget — FAITHFULLY MATCHING P1 v9.0 (p1_RTFMP_hou.py)
...
# RTFMP m_prime ≈ 195; cap at 200 (full P1 budget) for adequate power
...
#       B_null_min = ⌈π̂₀ · m' / α⌉ − 1
#   RTFMP (m'≈195, π̂₀=0.8, α=0.05): min ≈ 3119 — infeasible per level.
...
def run_original_data(n_workers: int = 1) -> dict:
    """
    Run all three methods on the original (unperturbed) RTFMP log.
...
def main():
    print("\n" + "=" * 100)
    print("RQ2 — SPECIFICATION QUALITY DEGRADATION: RTFMP  (v3.0)")
    ...
    # Section 4: summary
    print(f"\n{'='*100}")
    print("RQ2 — RTFMP COMPLETE  (v3.0 Joint Noise)")
    print(f"{'='*100}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            "RQ2 Specification Degradation — RTFMP v3.0  "
