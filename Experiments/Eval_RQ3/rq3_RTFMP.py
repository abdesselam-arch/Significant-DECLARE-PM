...
# INPUT_CSV = "../Phase 1 - KM Catalog Construction/Experiments data/CSV/Road_Traffic_Fine_Management_Process.csv"
# DECLARE_SPEC_FILE = "../Phase 1 - KM Catalog Construction/Experiments data/Experiments/Results/DECspec_RTFMP/phase0_declare_specification_CC.json"
# CLEAN_RESULTS_FILE = "../Phase 1 - KM Catalog Construction/Experiments data/Experiments/Results/RTFMP_ThreeHyp_SAM2/three_hypothesis_samfdr_results.json"
# OUTPUT_DIR = "../Phase 1 - KM Catalog Construction/Experiments data/Experiments/Results/RTFMP_RQ3_Direction"
INPUT_CSV = "Road_Traffic_Fine_Management_Process.csv"
DECLARE_SPEC_FILE = "phase0_RTFMP.json"
CLEAN_RESULTS_FILE = "RTFMP_ThreeHyp_SAM/three_hypothesis_samfdr_results.json"
...
if not PHASE1_IMPORTED:
    ...
    class CaseInfo:
        ...
        trace: List[str]          # Activity sequence (complete events only), outcome signal stripped
        vehicle_class: str        # vehicleClass attribute of the fine (case-level, first event)
    ...
    def adaptive_storey_pi0(pvals, q=0.05, lambda_max=0.80, delta=None,
        ...
        Matches p1_RTFMP.py — replaces Bootstrap-MSE storey_pi0_bootstrap.
    ...
    def extract_subgroups_from_case_data(case_data):
        """RTFMP subgroups: VehicleClass × DismissalType (matches p1_RTFMP.py)."""
...
def load_event_log(filepath: str) -> Dict[str, CaseInfo]:
    """
    Load RTFMP event log and extract case information.
    ...
    Column mapping:
        ...
        dismissal            → dismissal type of the fine (case-level, first non-null)

    Outcome determination (Teinemaa et al. TKDE 2019 — canonical RTFM binary outcome):
        Class 1 (Sent for Credit Collection / Deviant) —
            'Send for Credit Collection' present anywhere in the case.
        Class 0 (No Credit Collection / Normal) —
            'Send for Credit Collection' absent from the case.

    Lifecycle filter: only 'complete' events are retained; retaining 'start' rows
    would double-count every activity, distorting trace lengths and pattern counts.

    Outcome-signal removal: 'Send for Credit Collection' is stripped from the trace
    after labelling to prevent trivially dominant DECLARE constraints.
    """
    print(f"\n{'='*100}")
    print("📊 LOADING RTFMP EVENT LOG")
    print(f"{'='*100}")
    ...
    for case_id, group in df.groupby('case:concept:name'):
        ...
        # Label: 1 if target activity appears anywhere, 0 otherwise
        ...
        # vehicleClass: case-level attribute — first non-null value
        vc_series = case_events['vehicleClass'].dropna() if 'vehicleClass' in case_events.columns else pd.Series([], dtype=object)
        vehicle_class = str(vc_series.iloc[0]) if len(vc_series) > 0 else 'UNKNOWN'

        # dismissal: case-level attribute — first non-null value
        dm_series = case_events['dismissal'].dropna() if 'dismissal' in case_events.columns else pd.Series([], dtype=object)
        dismissal_type = str(dm_series.iloc[0]) if len(dm_series) > 0 else 'UNKNOWN'
    ...
    if skipped:
        print(f"   ⚠️  Skipped {skipped:,} cases with empty trace after stripping")
...
def apply_P1_activity_label_noise(
    ...
    RTFMP note: the RTFMP log records ~10 activities post-stripping per case
    (median trace length ~4 complete events). Even single swaps destroy
    ChainSuccession/ChainResponse signal in the short fine-processing sequences
...
def apply_P2_class_label_noise(
    ...
    RTFMP note: the RTFMP log has imbalance ratio ~3.7:1 (No-Credit-Collection vs
    Sent-for-Credit-Collection). P2 at ε=0.10 flips ~15,000 of ~150,370 cases;
    the minority class (Sent for Credit Collection) loses signal rapidly under
    flipping due to its smaller share of the case population.
    """
    rng = np.random.RandomState(random_state)
...
def apply_P3_trace_truncation(
    ...
    RTFMP note: truncation simulates incomplete fine-processing records — a realistic
    failure mode where later administrative steps (e.g. Add Penalty, Send for
    Credit Collection) are not yet recorded. The RTFMP log has short complete-event
    traces (mean ~4 events post-stripping); at ε=0.20 many traces shrink to 1–2
    events, making Response/Succession binary constraints vacuous.
...
def run_phase1_on_perturbed_log(
    ...
    if not PHASE1_IMPORTED:
        raise RuntimeError(
            "run_phase1_on_perturbed_log requires the Phase 1 module (p1_RTFMP). "
...
def generate_rq3_visualizations(
    ...
    Log:       Road Traffic Fine Management Process — ~150K cases
    Outcome:   Sent for Credit Collection (deviant) vs. No (normal) — IR ≈ 3.7:1
...
{inf_text}
    Expected Degradation Patterns (RTFMP-specific):
      P1 (activity noise):  Structural axis degrades; ChainSuccession most fragile
                             due to short RTFMP complete-event traces (~4 events)
      P2 (class noise):     Discriminative axis degrades; inflection earlier (~0.05–0.10)
                             due to minority class dilution (IR≈3.7)
      P3 (truncation):      Both axes degrade; binary consequent constraints become
                             vacuous rapidly; Init/End immune
    """
...
def save_rq3_outputs(
    ...
    rpt.append("=" * 120)
    rpt.append("SCIENTIFIC INTERPRETATION — RTFMP LOG")
    rpt.append("=" * 120)
    rpt.append("")
    rpt.append("Dataset: Road Traffic Fine Management Process (van Dongen 2012, 4TU)")
    rpt.append("Outcome: Sent for Credit Collection (deviant, Class 1) vs. No (normal, Class 0)")
    rpt.append("Imbalance ratio: ~3.7:1 (majority=No-Credit-Collection; minority=Sent-for-Credit-Collection)")
    rpt.append("Trace length post-stripping: mean ~4 complete events per case")
    rpt.append("")
    ...
    rpt.append("  - Very short RTFMP complete-event traces (~4 events) amplify P1 sensitivity:")
    rpt.append("    a single swap in a 3-event trace destroys all ChainSuccession/ChainResponse signal")
    ...
    rpt.append("  - Discriminative axis degrades; inflection point expected at ε ≈ 0.05–0.10")
    rpt.append("    (minority class dilution at IR≈3.7 accelerates Δ_obs collapse)")
    rpt.append("  - Structural axis unaffected (trace structures preserved)")
    rpt.append("  - Expected: 'Both' → 'Structural-only', phase transition at ε ≈ 0.05–0.10")
    rpt.append("")
    rpt.append("P3 (Trace truncation — targets both axes via vacuity):")
    rpt.append("  - Simulates incomplete fine-processing records (missing late administrative steps)")
    rpt.append("  - Vacuity rate rises rapidly; Response/Succession family most affected")
    rpt.append("  - Init/End constraints immune (unary, position-based)")
    rpt.append("  - At ε=0.20: many traces reduced to 1–2 events — binary consequents vacuous")
    rpt.append("    for the majority of cases; q-values inflate across both axes")
    rpt.append("")
    rpt.append("=" * 120)
...
def main(n_jobs_outer: int = 1):
    ...
    print("LOG: Road Traffic Fine Management Process — Sent for Credit Collection outcome — IR ≈ 3.7:1")
    print("=" * 100)
