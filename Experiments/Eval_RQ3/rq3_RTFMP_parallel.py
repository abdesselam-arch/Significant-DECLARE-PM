...
INPUT_CSV = "Road_Traffic_Fine_Management_Process.csv"
DECLARE_SPEC_FILE = "phase0_RTFMP.json"
CLEAN_RESULTS_FILE = "RTFMP_ThreeHyp_SAM_Parallel/three_hypothesis_samfdr_results.json"
OUTPUT_DIR = "RTFMP_RQ3_Direction"
...
# Import from Phase 1 script (RTFMP version)
try:
    from P1_SDSM.p1_RTFMP_parallel import (
    ...
    print("✓ Phase 1 RTFMP module imported successfully")
...
if not PHASE1_IMPORTED:
    ...
    def _clean_categorical(val) -> str:
        """Normalise a raw categorical column value to a clean string token."""
        if val is None or (isinstance(val, float) and np.isnan(val)):
            return 'UNKNOWN'
        s = str(val).strip()
        return s if s not in ('', 'nan', 'None', 'NaN') else 'UNKNOWN'

    @dataclass
    class CaseInfo:
        """Information about a single Road Traffic Fine Management Process case."""
        case_id: str
        outcome: int              # 1 = Sent for Credit Collection (Deviant), 0 = No Credit Collection (Normal)
        trace: List[str]          # Activity sequence with 'Send for Credit Collection' stripped
        vehicle_class: str        # vehicleClass attribute of the fine (case-level, first non-null)
        dismissal_type: str       # dismissal attribute value (case-level, first non-null)
    ...
    def adaptive_storey_pi0(pvals, q=0.05, lambda_max=0.80, delta=None,
        ...
        Matches p1_Sepsis.py — replaces Bootstrap-MSE storey_pi0_bootstrap.
    ...
    def extract_subgroups_from_case_data(case_data):
        """RTFMP subgroups: VehicleClass × DismissalType (matches p1_RTFMP_parallel.py)."""
        case_to_subgroups = {}; subgroup_to_cases = {}
        _sentinel = ('UNKNOWN', '', 'nan', 'None', 'NaN')
        for cid, case in case_data.items():
            sgs = []
            if case.vehicle_class not in _sentinel:
                sg = f"VehicleClass_{case.vehicle_class.replace(' ', '_')}"
                sgs.append(sg); subgroup_to_cases.setdefault(sg, set()).add(cid)
            if case.dismissal_type not in _sentinel:
                sg = f"Dismissal_{case.dismissal_type.replace(' ', '_')}"
                sgs.append(sg); subgroup_to_cases.setdefault(sg, set()).add(cid)
...
RQ3_CONFIG = {
    ...
    # RTFMP BUDGET NOTE: The RTFMP log has 150,370 cases — 143× larger than Sepsis
    # (~1,050 cases). Each permutation resample iterates over all n cases, so wall
    # time per resample scales linearly with n. Budgets are reduced vs. Sepsis to
    # keep the per-replicate compute tractable on standard HPC nodes, while
    # maintaining statistical adequacy:
    #   • Large n gives inherently stable p-values — fewer resamples needed.
    #   • Resolution: 1/(B_null+1) = 1/76 ≈ 0.013 << α=0.05 ✓
    #   • Test-power is n-driven, not B-driven, for these permutation statistics.
    'B_label': 500,    # B₁: label permutation resamples (Sepsis: 2000; RTFMP: 500)
    'B_trace': 200,    # B₂: trace permutation resamples (Sepsis: 1000; RTFMP: 200)
    'fdr_alpha': 0.05, # Target FDR level α
    'fdr_method': 'BH',
    # D9: Empirical calibration budget — resolution 1/(B_null+1) = 1/76 ≈ 0.013 << α
    # Wall-time estimate (12 cores): B_null × (B1_null + 2×B2_null) / 12
    #   ≈ 75 × (25 + 50) / 12 ≈ 469 per-task permutations → feasible
    'B_null':  75,    # double-null replicates for T_F empirical calibration
    'B1_null': 25,    # label perm budget per null replicate
    'B2_null': 25,    # trace perm budget per null replicate per class
...
# ============================================================================
# BUDGET TIERS — ε-LEVEL-SPECIFIC PERMUTATION BUDGETS (RTFMP-SCALED)
# ============================================================================
#
# Scientific justification (RTFMP-specific):
#   n=150,370 cases → each permutation resample is O(n) ≈ 143× more expensive
#   than Sepsis. Budgets are further reduced at high ε where degradation is
#   large and unambiguous and statistical precision on p-values is not needed.
#
#   All budgets satisfy the minimum resolution requirement:
#     1/(B_null+1) << α=0.05
#     At B_null=30: resolution = 1/31 ≈ 0.032 << 0.05 ✓ (most stringent tier)
#     At B_null=75: resolution = 1/76 ≈ 0.013 << 0.05 ✓ (least stringent tier)
#
# Paper disclosure (one sentence):
#   "For the RTFMP log (150,370 cases), permutation budgets were scaled relative
#    to Sepsis to maintain tractable wall-time: B₁∈[200,500], B₂∈[75,200],
#    B_null∈[30,75], tiered by ε level; resolution 1/(B_null+1) ≤ 0.032 << α=0.05."

EPS_BUDGET_TIERS: Dict[float, Dict[str, int]] = {
    0.02: {'B_label': 500, 'B_trace': 200, 'B_null': 75, 'B1_null': 25, 'B2_null': 25},
    0.05: {'B_label': 500, 'B_trace': 200, 'B_null': 75, 'B1_null': 25, 'B2_null': 25},
    0.10: {'B_label': 400, 'B_trace': 150, 'B_null': 50, 'B1_null': 20, 'B2_null': 20},
    0.20: {'B_label': 300, 'B_trace': 100, 'B_null': 40, 'B1_null': 15, 'B2_null': 15},
    0.30: {'B_label': 200, 'B_trace':  75, 'B_null': 30, 'B1_null': 12, 'B2_null': 12},
}
...
def load_event_log(filepath: str) -> Dict[str, CaseInfo]:
    """
    Load Road Traffic Fine Management Process event log and extract case information.

    Column mapping (de Leoni & Mannhardt 2015 / Teinemaa et al. TKDE 2019 traffic):
        case:concept:name  → case identifier (fine ID)
        time:timestamp     → event timestamp
        concept:name       → activity name (~11 unique activities)
        vehicleClass       → vehicle category of the fine (case-level, first non-null)
        dismissal          → dismissal attribute (case-level, first non-null)

    Outcome determination (Teinemaa et al. TKDE 2019 traffic_fines_1):
        Class 1 (Deviant)  — 'Send for Credit Collection' present anywhere in trace.
        Class 0 (Normal)   — 'Send for Credit Collection' absent.

    Outcome-signal removal: 'Send for Credit Collection' is stripped from the trace
    AFTER labelling. Retaining it would produce trivially dominant constraints
    (100% Class 1 / 0% Class 0) that encode the label rather than process behaviour.

    Scale note: 150,370 cases, 561,470 events. IR ≈ 1.55 (nearly balanced).
    RTFMP traces are short (median ~4 events post-stripping for the fine escalation
    path; mean ~3.7); Init/End constraints are very prevalent.
    """
    print(f"\n{'='*100}")
    print("📊 LOADING RTFMP EVENT LOG (Road Traffic Fine Management Process)")
    print(f"{'='*100}")
    ...
    for case_id, group in df.groupby('case:concept:name'):
        ...
        # Label: 1 if 'Send for Credit Collection' appears anywhere, 0 otherwise
        outcome = 1 if TARGET_ACTIVITY in all_activities else 0

        # Strip outcome signal to prevent trivially dominant patterns
        trace = [a for a in all_activities if a != TARGET_ACTIVITY]
        if len(trace) == 0:
            continue

        activity_index = precompute_activity_index(trace, case_id=str(case_id))

        # vehicleClass: case-level — take first non-null value
        vc_series = case_events['vehicleClass'].dropna() \
            if 'vehicleClass' in case_events.columns else pd.Series([], dtype=object)
        vehicle_class = _clean_categorical(vc_series.iloc[0]) if len(vc_series) > 0 else 'UNKNOWN'

        # dismissal: case-level — take first non-null value
        dm_series = case_events['dismissal'].dropna() \
            if 'dismissal' in case_events.columns else pd.Series([], dtype=object)
        dismissal_type = _clean_categorical(dm_series.iloc[0]) if len(dm_series) > 0 else 'UNKNOWN'
...
# def load_clean_specification(results_file: str) -> Set[str]:
#     """
#     Load the significant specification S_clean from the clean Phase 1 run.

#     Returns: set of pattern_id strings for all 'Both' category patterns.
#     """
...
#     # Also build a mapping for category tracking
#     all_pattern_categories = {}
#     for p in clean_results.get('all_patterns', []):
#         all_pattern_categories[p['pattern_id']] = p.get('significance_category', 'Neither')
...
def apply_P1_activity_label_noise(
    ...
    RTFMP note: the RTFMP log has ~11 distinct activities post-outcome-stripping
    (median trace length ~4 events; mean ~3.7). Traces are very short — a single
    swap in a 3-event trace permutes 67% of positions, making ChainSuccession and
    ChainResponse extremely sensitive to P1 even at small ε. Init/End constraints
    are position-based and also highly susceptible to head/tail swaps.
    P1 inflection point expected earlier than in longer-trace logs.
    """
    ...
    for cid, case in case_data.items():
        ...
        perturbed[cid] = CaseInfo(
            ...
            vehicle_class=case.vehicle_class, dismissal_type=case.dismissal_type,
...
def apply_P2_class_label_noise(
    ...
    RTFMP note: the RTFMP log has IR ≈ 1.55 (91,357 normal vs 59,013 deviant),
    nearly balanced. P2 at ε=0.10 flips ~15,037 of 150,370 cases; at ε=0.20
    flips ~30,074. Because both classes are large, Δ_obs degrades more slowly
    than in imbalanced logs — inflection point expected later (ε ≈ 0.15–0.20)
    vs. the ε ≈ 0.05–0.10 seen in minority-class-dominated logs.
    """
    rng = np.random.RandomState(random_state)
...
def apply_P3_trace_truncation(
    ...
    *random missing-data process at the case level* rather than a deterministic
    ...
    RTFMP note: at ε=0.20, ~30,074 of 150,370 cases are truncated. RTFMP traces
    are already very short (mean ~3.7 events post-stripping); truncating a 3-event
    trace to ⌈0.8·3⌉=3 leaves it unchanged, so P3 impact concentrates on the
    minority of longer cases (the fine escalation path: Create Fine → Insert
    Fine Notification → Add penalty → Payment → …). Vacuity will rise sharply
    for Succession/ChainSuccession but Init remains immune (first event is
    almost always 'Create Fine' regardless of truncation).
    """
    rng = np.random.RandomState(random_state)
    perturbed = {}
...
def run_phase1_on_perturbed_log(
    ...
    if not PHASE1_IMPORTED:
        raise RuntimeError(
            "run_phase1_on_perturbed_log requires the Phase 1 module (p1_RTFMP_parallel). "
            "The import failed — check that P1_SDSM/p1_RTFMP_parallel.py is on the path "
            "and fix the import error before running RQ3. (D7)"
        )
...
def generate_rq3_visualizations(
    ...
    summary = f"""
    RQ3 DIRECTION 2 — SENSITIVITY ANALYSIS SUMMARY  [RTFMP Log]
    ════════════════════════════════════════════════════════════════════════
    Framework: Sommers et al. (2025) Model-Transformation Perturbation
    Pipeline:  Dual-Axis Storey FDR (v8.0-DUAL-AXIS-STOREY-FDR)
    Log:       Road Traffic Fine Management Process (de Leoni & Mannhardt 2015)
               150,370 cases, 561,470 events
    Outcome:   Sent for Credit Collection (deviant) vs. No Credit Collection (normal)
               IR ≈ 1.55:1 (nearly balanced: deviant=59,013; normal=91,357)
...
{inf_text}
    Expected Degradation Patterns (RTFMP-specific):
      P1 (activity noise):  Structural axis degrades; ChainSuccession fragile at low ε
                             due to short post-stripping traces (~3.7 events mean);
                             discriminative axis less affected (IR≈1.55, balanced signal)
      P2 (class noise):     Discriminative axis degrades; later inflection (~0.10–0.20)
                             vs. imbalanced logs due to near-balanced class ratio (IR=1.55)
      P3 (truncation):      Both axes degrade; short traces make vacuity rise sharply even
                             at low ε; Init/End patterns immune; simulates partial fine records
    """
...
def save_rq3_outputs(
    ...
    rpt.append("=" * 120)
    rpt.append("SCIENTIFIC INTERPRETATION — ROAD TRAFFIC FINE MANAGEMENT PROCESS LOG")
    rpt.append("=" * 120)
    rpt.append("")
    rpt.append("Dataset: Road Traffic Fine Management Process (de Leoni & Mannhardt 2015,")
    rpt.append("         4TU doi:10.4121/uuid:270fd440-1057-4fb9-89a9-b699b47990f5)")
    rpt.append("Outcome: Sent for Credit Collection (deviant, Class 1) vs.")
    rpt.append("         No Credit Collection (normal, Class 0)")
    rpt.append("Scale:   150,370 cases, 561,470 events")
    rpt.append("Imbalance ratio: ~1.55:1 (normal=91,357; deviant=59,013) — nearly balanced")
    rpt.append("Trace length post-stripping: mean ~3.7, median ~4 events")
    rpt.append("")
    rpt.append("Expected differential degradation patterns (RTFMP-specific):")
    rpt.append("")
    rpt.append("P1 (Activity label noise — targets H₀ˢ):")
    rpt.append("  - Structural axis degrades monotonically in ε")
    rpt.append("  - RTFMP traces are extremely short (median 4 events): a single swap in a")
    rpt.append("    3-event trace permutes 67% of positions, making ChainSuccession and")
    rpt.append("    Init/End highly sensitive to P1 even at ε=0.02")
    rpt.append("  - Inflection point expected earlier than in longer-trace logs (Sepsis, BPI)")
    ...
    rpt.append("  - Discriminative axis degrades; inflection point expected LATER than Sepsis")
    rpt.append("    (ε ≈ 0.15–0.20 vs 0.05–0.10) because IR ≈ 1.55 — both classes are large")
    rpt.append("  - At ε=0.10: ~15,037 of 150,370 cases flipped; Δ_obs decays slowly due to")
    rpt.append("    large class sizes on both sides — genuine signal is diluted proportionally")
    rpt.append("  - At ε=0.20: ~30,074 cases flipped — still only 33% of minority class")
    rpt.append("  - Structural axis unaffected (trace structures preserved)")
    rpt.append("  - Expected: 'Both' → 'Structural-only', later phase transition vs. Sepsis")
    rpt.append("")
    rpt.append("P3 (Trace truncation — targets both axes via vacuity):")
    rpt.append("  - Simulates incomplete fine management recording (e.g., missing payment events)")
    rpt.append("  - Effect concentrated on longer traces (fine escalation path)")
    rpt.append("  - Traces of length ≤3 events: truncation to ⌈0.8·L⌉ events changes very")
    rpt.append("    little (⌈0.8·3⌉=3 = same trace) — P3 selectively hits longer cases")
    rpt.append("  - Init immune ('Create Fine' almost always first event regardless)")
    rpt.append("  - Succession/ChainSuccession most affected; NotResponse resilient")
    rpt.append("  - Vacuity rate expected to rise non-linearly (hits only the tail of the")
    rpt.append("    length distribution; short-trace majority largely unaffected)")
    rpt.append("")
    rpt.append("=" * 120)
...
def main(n_jobs_outer: int = 1, args=None):
    ...
    print("LOG: Sepsis Cases (Mannhardt 2016) — Return ER outcome — IR ≈ 3.9:1")
    print("=" * 100)
