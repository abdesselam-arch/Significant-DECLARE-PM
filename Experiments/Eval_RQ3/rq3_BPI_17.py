...
P3 — Trace truncation (RIe_mi): Retain first ⌈(1−ε)·L⌉ events per trace.
...
# INPUT_CSV = "../Phase 1 - KM Catalog Construction/Experiments data/CSV/BPI_Challenge_17.csv"
# DECLARE_SPEC_FILE = "../Phase 1 - KM Catalog Construction/Experiments data/Experiments/Results/DECspec_BPI17/phase0_BPI17.json"
# CLEAN_RESULTS_FILE = "../Phase 1 - KM Catalog Construction/Experiments data/Experiments/Results/BPI17_ThreeHyp_SAM/three_hypothesis_samfdr_results.json"
# OUTPUT_DIR = "../Phase 1 - KM Catalog Construction/Experiments data/Experiments/Results/BPI17_RQ3_Direction"
INPUT_CSV = "BPI_Challenge_17.csv"
DECLARE_SPEC_FILE = "phase0_BPI17.json"
CLEAN_RESULTS_FILE = "BPI17_ThreeHyp_SAM/three_hypothesis_samfdr_results.json"
OUTPUT_DIR = "BPI17_RQ3_Direction"
...
if not PHASE1_IMPORTED:
    ...
    class CaseInfo:
        ...
        case_id: str
        outcome: int              # 1 = Not-Accepted (O_Refused/O_Cancelled, Deviant), 0 = Accepted (O_Accepted, Normal)
        trace: List[str]          # Activity sequence (complete events only), terminal O_ events stripped
        loan_goal: str            # case:LoanGoal attribute (case-level)
        application_type: str     # case:ApplicationType attribute (case-level)
    ...
    def adaptive_storey_pi0(pvals, q=0.05, lambda_max=0.80, delta=None,
        ...
        Matches p1_BPI_17.py — replaces Bootstrap-MSE storey_pi0_bootstrap.
    ...
    def extract_subgroups_from_case_data(case_data):
        """BPI-17 subgroups: LoanGoal × ApplicationType (matches p1_BPI_17.py)."""
        _sentinel = ('UNKNOWN', '', 'nan', 'None', 'none', 'NaN')
        case_to_subgroups = {}; subgroup_to_cases = {}
        for cid, case in case_data.items():
            sgs = []
            if case.loan_goal not in _sentinel:
                sg = f"LoanGoal_{case.loan_goal.replace(' ', '_')}"
                sgs.append(sg); subgroup_to_cases.setdefault(sg, set()).add(cid)
            if case.application_type not in _sentinel:
                sg = f"AppType_{case.application_type.replace(' ', '_')}"
                sgs.append(sg); subgroup_to_cases.setdefault(sg, set()).add(cid)
...
def load_event_log(filepath: str) -> Dict[str, CaseInfo]:
    ...
    Column mapping:
        ...
        lifecycle:transition → event lifecycle state; only 'complete' events retained
        case:LoanGoal        → loan goal (case-level attribute)
        case:ApplicationType → application type (case-level attribute)

    Outcome determination (canonical BPI-17 binary outcome, Teinemaa et al. TKDE 2019):
        Class 0 (Accepted / Normal) —
            terminal offer event is 'O_Accepted'.
        Class 1 (Not-Accepted / Deviant) —
            terminal offer event is 'O_Refused' or 'O_Cancelled'.
        Cases with no terminal offer event are excluded (incomplete traces).

    Lifecycle filter: only 'complete' events are retained; the BPI-17 log records
    both 'start' and 'complete' lifecycle transitions — retaining both would
    double-count every activity, distorting trace lengths and pattern counts.

    Outcome-signal removal: all three terminal offer events ('O_Accepted',
    'O_Refused', 'O_Cancelled') are stripped from the trace after labelling to
    prevent trivially dominant DECLARE constraints involving the outcome event.
    """
    print(f"\n{'='*100}")
    print("📊 LOADING BPI CHALLENGE 2017 EVENT LOG")
    print(f"{'='*100}")

    TERMINAL_OFFER_OUTCOMES = {'O_Accepted', 'O_Refused', 'O_Cancelled'}
    ...
    for case_id, group in df.groupby('case:concept:name'):
        ...
        # Identify terminal offer event (last activity in TERMINAL_OFFER_OUTCOMES)
        terminal_offer = None
        for act in reversed(all_activities):
            if act in TERMINAL_OFFER_OUTCOMES:
                terminal_offer = act
                break

        if terminal_offer is None:
            # No terminal offer event — exclude case (outcome undetermined)
            skipped_no_terminal += 1
            continue

        # Outcome: 0=Accepted (O_Accepted), 1=Not-Accepted (O_Refused/O_Cancelled)
        outcome = 0 if terminal_offer == 'O_Accepted' else 1

        # Strip all terminal offer events from trace to prevent trivially dominant patterns
        trace = [a for a in all_activities if a not in TERMINAL_OFFER_OUTCOMES]
        if len(trace) == 0:
            skipped += 1
            continue
        ...
        # case:LoanGoal — case-level attribute (constant per case; take first non-null)
        lg_series = case_events['case:LoanGoal'].dropna() if 'case:LoanGoal' in case_events.columns else pd.Series([], dtype=object)
        loan_goal = str(lg_series.iloc[0]) if len(lg_series) > 0 else 'UNKNOWN'

        # case:ApplicationType — case-level attribute (constant per case; take first non-null)
        at_series = case_events['case:ApplicationType'].dropna() if 'case:ApplicationType' in case_events.columns else pd.Series([], dtype=object)
        application_type = str(at_series.iloc[0]) if len(at_series) > 0 else 'UNKNOWN'
    ...
    if skipped_no_terminal:
        print(f"   ⚠️  Skipped {skipped_no_terminal:,} cases with no terminal offer event (outcome undetermined)")
    if skipped:
        print(f"   ⚠️  Skipped {skipped:,} cases with empty trace after stripping terminal offer events")
    n1 = sum(1 for c in case_data.values() if c.outcome == 1)
    n0 = len(case_data) - n1
    print(f"   ✓ Processed {len(case_data):,} cases")
    print(f"   Class 1 (Not-Accepted: O_Refused/O_Cancelled / Deviant): {n1:,} ({n1/len(case_data)*100:.1f}%)")
    print(f"   Class 0 (Accepted: O_Accepted / Normal):                  {n0:,} ({n0/len(case_data)*100:.1f}%)")
...
def apply_P1_activity_label_noise(
    ...
    BPI-17 note: the BPI-17 log records loan application processes with A_
    (application) and O_ (offer) event types. After stripping terminal offer events,
    traces contain primarily A_ activities (e.g. A_Submitted, A_Complete, A_Accepted).
    Even single swaps destroy ChainSuccession/ChainResponse signal in the sequential
    ...
    for cid, case in case_data.items():
        ...
        perturbed[cid] = CaseInfo(
            ...
            loan_goal=case.loan_goal, application_type=case.application_type,
...
def apply_P2_class_label_noise(
    ...
    BPI-17 note: the BPI-17 log has imbalance ratio ~3:1 (Not-Accepted vs Accepted).
    P2 at ε=0.10 flips ~3,000 of ~31,500 cases; the minority class (Accepted)
    loses signal rapidly under flipping due to its smaller share of the case population.
    """
    rng = np.random.RandomState(random_state)
...
def apply_P3_trace_truncation(
    ...
    """
    P3 — Trace truncation (operationalizes RIe_mi for tail events).

    For each trace of length L, retain only the first ⌈(1−ε)·L⌉ events.

    Targets: Both axes via vacuity inflation.
    Primarily reduces n_applicable for Response, AlternateResponse,
    ChainResponse, and Succession families, where the consequent
    typically occurs late.

    BPI-17 note: truncation simulates incomplete loan application records — a realistic
    failure mode where later workflow steps (e.g. O_Created, W_ work items) are not
    yet recorded. The BPI-17 log has longer traces than RTFMP (mean ~20+ complete
    events post-stripping); at ε=0.20 traces lose their later O_ offer-creation
    steps, making Response/Succession binary constraints vacuous for offer-related
    activity pairs. A_ application events are more front-loaded and relatively immune.
    Init/End constraints are largely immune (position-based).
    """
    perturbed = {}
    ...
    for cid, case in case_data.items():
        ...
        truncated_trace = case.trace[:retain_length]
        activity_index = precompute_activity_index(truncated_trace, case_id=cid)

        perturbed[cid] = CaseInfo(
            case_id=cid, outcome=case.outcome, trace=truncated_trace,
            loan_goal=case.loan_goal, application_type=case.application_type,
            start_timestamp=case.start_timestamp,
            activity_index=activity_index,
        )
...
def run_phase1_on_perturbed_log(
    ...
    if not PHASE1_IMPORTED:
        raise RuntimeError(
            "run_phase1_on_perturbed_log requires the Phase 1 module (p1_BPI_17). "
...
def _rq3_worker(
    ...
    local_config = {**pipeline_config, 'random_state': seed}

    # Fix 3 (minor): silence tqdm inside loky workers to avoid garbled SLURM logs.
...
def run_rq3_experiment(
    ...
    # Fix 4 (safety): guard against nested loky oversubscription when called
    # programmatically (e.g. from a notebook) without setting config['n_jobs']=1.
    if n_jobs_outer != 1 and config.get('n_jobs', -1) != 1:
        warnings.warn(
            "n_jobs_outer != 1 with config['n_jobs'] != 1 will cause nested loky "
            "oversubscription. Forcing config['n_jobs'] = 1.",
            RuntimeWarning, stacklevel=2,
        )
        config = {**config, 'n_jobs': 1}
    ...
    pipeline_config = {
        ...
        'B_null':       config.get('B_null',  200),  # D10 fix — forward empirical calibration budget
        'B1_null':      config.get('B1_null', 75),   # D10 fix
        'B2_null':      config.get('B2_null', 75),   # D10 fix
        'n_jobs':       config.get('n_jobs',  -1),   # D10 fix
    }
...
def generate_rq3_visualizations(
    ...
    summary = f"""
    RQ3 DIRECTION 2 — SENSITIVITY ANALYSIS SUMMARY  [BPI Challenge 2017 Log]
    ════════════════════════════════════════════════════════════════════════
    Framework: Sommers et al. (2025) Model-Transformation Perturbation
    Pipeline:  Dual-Axis Storey FDR (v8.0-DUAL-AXIS-STOREY-FDR)
    Log:       BPI Challenge 2017 — ~31.5K cases (loan applications)
    Outcome:   O_Accepted (normal, class 0) vs. O_Refused/O_Cancelled (deviant, class 1)
...
{inf_text}
    Expected Degradation Patterns (BPI-17-specific):
      P1 (activity noise):  Structural axis degrades; ChainSuccession most fragile
                             in sequential A_/O_ loan-processing workflows
      P2 (class noise):     Discriminative axis degrades; inflection expected ~0.10–0.20
                             (BPI-17 is relatively balanced; Δ_obs collapses more slowly)
      P3 (truncation):      Both axes degrade; offer-creation steps (O_ events) lost
                             first; Response/Succession on A_→O_ pairs become vacuous
    """
...
def save_rq3_outputs(
    ...
    rpt.append("=" * 120)
    rpt.append("SCIENTIFIC INTERPRETATION — BPI CHALLENGE 2017 LOG")
    rpt.append("=" * 120)
    rpt.append("")
    rpt.append("Dataset: BPI Challenge 2017 — Loan Application Process (van Dongen 2017, 4TU)")
    rpt.append("Outcome: O_Accepted (normal, Class 0) vs. O_Refused/O_Cancelled (deviant, Class 1)")
    rpt.append("Outcome determination: terminal offer event in {O_Accepted, O_Refused, O_Cancelled}")
    rpt.append("Trace length post-stripping: mean ~20+ complete events per case (longer than RTFMP)")
    rpt.append("")
    rpt.append("Expected differential degradation patterns (BPI-17-specific):")
    rpt.append("")
    rpt.append("P1 (Activity label noise — targets H₀ˢ):")
    rpt.append("  - Structural axis degrades monotonically in ε")
    rpt.append("  - Longer BPI-17 traces (vs. RTFMP) reduce P1 sensitivity per swap:")
    rpt.append("    a single swap in a 20-event trace has less impact than in a 4-event trace")
    rpt.append("  - ChainSuccession/ChainResponse involving sequential A_/O_ steps most fragile")
    ...
    rpt.append("  - Discriminative axis degrades; inflection point expected at ε ≈ 0.10–0.20")
    rpt.append("    (BPI-17 is relatively balanced; Δ_obs collapses more slowly than for RTFMP)")
    rpt.append("  - Structural axis unaffected (trace structures preserved)")
    rpt.append("  - Expected: 'Both' → 'Structural-only', phase transition at ε ≈ 0.10–0.20")
    rpt.append("")
    rpt.append("P3 (Trace truncation — targets both axes via vacuity):")
    rpt.append("  - Simulates incomplete loan application records (missing later O_/W_ steps)")
    rpt.append("  - Vacuity rate rises; Response/Succession on A_→O_ pairs most affected")
    rpt.append("  - Init/End constraints immune (unary, position-based)")
    rpt.append("  - At ε=0.20: offer-creation steps removed for most cases —")
    rpt.append("    binary consequent constraints for A_→O_ pairs become vacuous")
    rpt.append("")
    rpt.append("=" * 120)
...
def main(n_jobs_outer: int = 1):
    ...
    print("LOG: BPI Challenge 2017 — Loan Application Process — O_Accepted vs. O_Refused/O_Cancelled")
    print("=" * 100)
    ...
    all_metrics = run_rq3_experiment(case_data, candidates_all, S_clean, RQ3_CONFIG,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--n-jobs-inner', type=int, default=1,
        help='Inner calibration parallelism per task (default: 1 — '
             'avoids oversubscription when --n-jobs > 1)')
    ...
    # Inner calibration: force n_jobs=1 when outer parallelism > 1 to avoid
    # nested oversubscription (each outer worker already uses its own cores)
    RQ3_CONFIG['n_jobs'] = args.n_jobs_inner

    main(n_jobs_outer=args.n_jobs)