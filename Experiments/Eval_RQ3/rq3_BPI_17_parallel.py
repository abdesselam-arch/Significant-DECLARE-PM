...
CLEAN_RESULTS_FILE = "BPI17_ThreeHyp_SAM_Parallel/three_hypothesis_samfdr_results.json"
...
# Import from Phase 1 script (BPI Challenge 2017 version)
try:
    from P1_SDSM.p1_BPI_17_parallel import (
...
if not PHASE1_IMPORTED:
    ...
    def _clean_categorical(raw) -> str:
        """Normalise a case-level categorical attribute; return 'UNKNOWN' if missing."""
    ...
    class CaseInfo:
        ...
        case_id: str
        outcome: int                   # 1 = Not-Accepted (O_Refused/O_Cancelled, Deviant), 0 = Accepted (Normal)
        trace: List[str]               # O_-lifecycle events, terminal offer activities stripped
        application_type: str          # case:ApplicationType (e.g., "New credit", "Limit raise")
        loan_goal: str                 # case:LoanGoal (e.g., "Home improvement", "Existing loan takeover")
    ...
    def extract_subgroups_from_case_data(case_data):
        """BPI 2017 subgroups: ApplicationType × LoanGoal (matches p1_BPI_17_parallel.py)."""
        ...
        for cid, case in case_data.items():
            ...
            if case.application_type not in _sentinel:
                sg = f"AppType_{case.application_type.replace(' ', '_').replace('/', '_')}"
                sgs.append(sg); subgroup_to_cases.setdefault(sg, set()).add(cid)
            if case.loan_goal not in _sentinel:
                sg = f"LoanGoal_{case.loan_goal.replace(' ', '_').replace('/', '_')}"
                sgs.append(sg); subgroup_to_cases.setdefault(sg, set()).add(cid)
...
RQ3_CONFIG = {
    ...
    # BPI-17 scale: 31,509 cases — ~30× Sepsis, ~5× cheaper than RTFMP.
    # Budgets reduced to keep per-cell wall time manageable while preserving
    # resolution: 1/(B_null+1) = 1/101 ≈ 0.010 << α=0.05 ✓
    'B_label': 1000,      # B₁: label permutation resamples  (Sepsis: 2000, RTFMP: 500)
    'B_trace':  500,      # B₂: trace permutation resamples  (Sepsis: 1000, RTFMP: 200)
    ...
    # Resolution: 1/(B_null+1) = 1/101 ≈ 0.010 << α=0.05 ✓
    'B_null':  100,   # double-null replicates for T_F empirical calibration  (Sepsis: 200, RTFMP: 75)
    'B1_null':  40,   # label perm budget per null replicate  (Sepsis: 75, RTFMP: 25)
    'B2_null':  40,   # trace perm budget per null replicate per class  (Sepsis: 75, RTFMP: 25)
...
#   "Permutation budgets were tiered by ε level: full budgets (B₁=1000,
#    B₂=500, B_null=100) at ε ≤ 0.05; reduced budgets (B₁=400, B₂=200,
#    B_null=40) at ε=0.30 where degradation is unambiguous; intermediate
#    budgets at ε=0.10 and ε=0.20. All tiers satisfy resolution
#    1/(B_null+1) << α=0.05 (minimum resolution ≈ 0.024 at B_null=40)."

EPS_BUDGET_TIERS: Dict[float, Dict[str, int]] = {
    0.02: {'B_label': 1000, 'B_trace': 500, 'B_null': 100, 'B1_null': 40, 'B2_null': 40},
    0.05: {'B_label': 1000, 'B_trace': 500, 'B_null': 100, 'B1_null': 40, 'B2_null': 40},
    0.10: {'B_label':  800, 'B_trace': 400, 'B_null':  75, 'B1_null': 30, 'B2_null': 30},
    0.20: {'B_label':  600, 'B_trace': 300, 'B_null':  60, 'B1_null': 25, 'B2_null': 25},
    0.30: {'B_label':  400, 'B_trace': 200, 'B_null':  40, 'B1_null': 20, 'B2_null': 20},
}
...
def load_event_log(filepath: str) -> Dict[str, CaseInfo]:
    ...
    Column mapping:
        ...
        concept:name           → activity name (26 unique activities across A_/O_/W_ namespaces)
        lifecycle:transition   → event lifecycle phase (filter: COMPLETE only)
        case:ApplicationType   → loan application type (subgroup dim 1)
        case:LoanGoal          → loan purpose (subgroup dim 2)

    Lifecycle filter: Applied to ALL events first (case-insensitive .str.upper() == 'COMPLETE'),
        exactly matching p1_BPI_17_parallel.py. The previous version filtered O_ events
        first and used a case-sensitive exact match, which zeroed out the entire log
        when the CSV stores 'complete' (lowercase).

    Outcome determination (Teinemaa et al. TKDE 2019 — bpic2017_accepted):
        The terminal O_ activity (last chronological COMPLETE O_ event) determines class:
        Class 0 (Accepted / Normal)      — terminal offer is 'O_Accepted'
        Class 1 (Not-Accepted / Deviant) — terminal offer is 'O_Refused' or 'O_Cancelled'
        Cases whose terminal offer is neither are excluded (ambiguous terminal state).

    Trace construction: ALL COMPLETE activities minus terminal offer acts.
        Matches p1_BPI_17_parallel.py exactly — traces include W_, A_, and non-terminal
        O_ events. The previous version retained only O_ events in the trace.

    Outcome-signal removal: Terminal offer activities {'O_Accepted', 'O_Refused',
        'O_Cancelled'} are stripped from the DECLARE trace after labelling.
        Retaining them would produce trivially dominant DECLARE constraints
        that encode the label directly rather than genuine process behaviour.

    Case-level attributes: Read from case_events (all rows), not offer_events,
        because case:ApplicationType and case:LoanGoal are often populated only
        on A_/W_ rows in BPI17 CSV exports.
    """
    print(f"\n{'='*100}")
    print("📊 LOADING BPI CHALLENGE 2017 EVENT LOG")
    print(f"{'='*100}")

    TERMINAL_OFFER_ACTS = {'O_Accepted', 'O_Refused', 'O_Cancelled'}

    df = pd.read_csv(filepath, low_memory=False)
    df = df.dropna(subset=['case:concept:name'])
    print(f"   ✓ Loaded {len(df):,} raw events from {df['case:concept:name'].nunique():,} cases")

    # FIX: lifecycle filter applied to ALL events first, case-insensitive — matches p1
    if 'lifecycle:transition' in df.columns:
        n_before = len(df)
        df = df[df['lifecycle:transition'].str.upper() == 'COMPLETE'].copy()
        print(f"   ✓ Lifecycle filter (COMPLETE only): {n_before:,} → {len(df):,} events retained")
    else:
        print("   ⚠️  No lifecycle:transition column found — no lifecycle filter applied")
    ...
    for case_id, group in df.groupby('case:concept:name'):
        ...
        # FIX: trace = ALL COMPLETE activities minus terminal offer acts (matches p1)
        all_activities = case_events['concept:name'].tolist()
        trace = [a for a in all_activities if a not in TERMINAL_OFFER_ACTS]
        if len(trace) == 0:
            skipped['empty_trace_after_strip'] += 1
            continue

        # Terminal offer determination from O_ events (labelling only)
        offer_events = case_events[case_events['concept:name'].str.startswith('O_', na=False)]
        if offer_events.empty:
            skipped['no_offer_events'] += 1
            continue
        terminal_offer = offer_events.iloc[-1]['concept:name']
        if terminal_offer == 'O_Accepted':
            outcome = 0   # Normal
        elif terminal_offer in {'O_Refused', 'O_Cancelled'}:
            outcome = 1   # Deviant (Not-Accepted)
        else:
            # Ambiguous terminal state (e.g., O_Create Offer, O_Returned) — skip
            skipped['ambiguous_terminal'] += 1
            continue

        activity_index = precompute_activity_index(trace, case_id=str(case_id))

        # FIX: read case-level attributes from case_events (all rows), not offer_events
        app_type_raw = case_events['case:ApplicationType'].iloc[0] \
            if 'case:ApplicationType' in case_events.columns else float('nan')
        application_type = _clean_categorical(app_type_raw)

        loan_goal_raw = case_events['case:LoanGoal'].iloc[0] \
            if 'case:LoanGoal' in case_events.columns else float('nan')
        loan_goal = _clean_categorical(loan_goal_raw)
    ...
    if len(case_data) == 0:
        raise RuntimeError(
            f"load_event_log: case_data is empty after all filters. "
            f"Skipped counts: {dict(skipped)}. "
            f"Check that 'lifecycle:transition' values in the CSV are 'complete' or 'COMPLETE'."
        )

    n1 = sum(1 for c in case_data.values() if c.outcome == 1)
    n0 = len(case_data) - n1
    print(f"\n📊 Outcome Distribution (Teinemaa et al. TKDE 2019 — bpic2017_accepted):")
...
def apply_P1_activity_label_noise(
    ...
    Sepsis note: the Sepsis log records 15 activities post-outcome-stripping
    (median trace length ~13). Even single swaps in the short triage sequence
    (ER Registration → Leucocytes → CRP) destroy ChainSuccession signal.
    """
...
def apply_P2_class_label_noise(
    ...
    BPI-17 note: the BPI-17 log has IR ≈ 1.47:1 (Not-Accepted=18,747 vs
    Accepted=12,762). P2 at ε=0.10 flips ~3,151 cases; the near-balanced ratio
    means both classes absorb roughly equal noise, so Δ_obs shrinks gradually
    rather than collapsing — inflection expected later (~ε=0.15–0.20).
    """
    rng = np.random.RandomState(random_state)
...
def apply_P3_trace_truncation(
    ...
    *random missing-data process at the case level* rather than a deterministic
    ...
    Sepsis note: at ε=0.20, ~210 of 1,050 cases are truncated; median
    retained length ≈ 11 events for truncated cases.
    """
    rng = np.random.RandomState(random_state)
    perturbed = {}
...
def run_phase1_on_perturbed_log(
    ...
    if not PHASE1_IMPORTED:
        raise RuntimeError(
            "run_phase1_on_perturbed_log requires the Phase 1 module (p1_BPI_17_parallel). "
...
def generate_rq3_visualizations(
    ...
    Log:       BPI Challenge 2017 (van Dongen 2017) — 31,509 cases; O_-traces
    Outcome:   Not-Accepted (deviant) vs. Accepted (normal) — IR ≈ 1.47:1
...
{inf_text}
    Expected Degradation Patterns (BPI-17-specific):
      P1 (activity noise):  Structural axis degrades; compact O_ vocabulary amplifies
                             per-swap disruption; discriminative axis unaffected
      P2 (class noise):     Discriminative axis degrades; inflection LATER (~0.15–0.20)
                             vs. imbalanced logs due to near-balanced classes (IR=1.47)
      P3 (truncation):      Both axes degrade; compact O_ traces make vacuity rise sharply;
                             Init/End immune; simulates incomplete offer-sequence records
    """
...
def save_rq3_outputs(
    ...
    rpt.append("=" * 120)
    rpt.append("")
    rpt.append("Dataset: BPI Challenge 2017 (van Dongen 2017, 4TU doi:10.4121/uuid:5f3067df)")
    rpt.append("         Dutch financial institution — loan application process")
    rpt.append("Scale:   31,509 cases, 1,202,267 total events; O_-lifecycle traces used")
    rpt.append("Outcome: Not-Accepted (deviant, Class 1) vs. Accepted (normal, Class 0)")
    rpt.append("         Labelled by terminal O_ activity per Teinemaa et al. (TKDE 2019)")
    rpt.append("         Class 1: 18,747 (59.5%); Class 0: 12,762 (40.5%); IR ≈ 1.47:1")
    rpt.append("Trace:   COMPLETE O_-lifecycle events, terminal offers stripped")
    rpt.append("         Compact vocabulary (~8 unique O_ activities post-stripping)")
    rpt.append("")
    ...
    rpt.append("  - Compact O_ vocabulary amplifies P1 impact: swapping within a small")
    rpt.append("    alphabet (e.g., O_Sent ↔ O_Returned) disrupts ChainSuccession more")
    rpt.append("    severely than in logs with large activity sets")
    ...
    rpt.append("  - Discriminative axis degrades; inflection point expected LATER (~ε=0.15–0.20)")
    rpt.append("    than imbalanced logs because IR≈1.47 means both classes are well-populated")
    rpt.append("  - At ε=0.10: ~3,151 cases flipped; deviant pool (18,747) loses only 10%")
    rpt.append("    of genuine signal → Δ_obs shrinks gradually rather than collapsing")
    rpt.append("  - Structural axis unaffected (trace structures preserved)")
    rpt.append("  - Expected: 'Both' → 'Structural-only', gradual phase transition ε ≈ 0.15–0.20")
    rpt.append("")
    rpt.append("P3 (Trace truncation — targets both axes via vacuity):")
    rpt.append("  - Simulates incomplete offer-sequence recording (realistic BPI-17 failure mode)")
    rpt.append("  - Compact O_ traces mean vacuity rises earlier than in longer-trace logs")
    ...
    rpt.append("  - Both axes degrade together; Succession family most sensitive")
    rpt.append("")
    rpt.append("=" * 120)
...
def main(n_jobs_outer: int = 1, args=None):
    ...
    print("LOG: BPI Challenge 2017 (van Dongen 2017) — Not-Accepted outcome — IR ≈ 1.47:1")
    print("=" * 100)
