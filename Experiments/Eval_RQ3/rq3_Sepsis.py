...
INPUT_CSV = "Sepsis_EL.csv"
DECLARE_SPEC_FILE = "phase0_Sepsis.json"
CLEAN_RESULTS_FILE = "Sepsis_ThreeHyp_SAM/three_hypothesis_samfdr_results.json"
OUTPUT_DIR = "Sepsis_RQ3_Direction"
...
# Import from Phase 1 script (Sepsis version)
try:
    from P1_SDSM.p1_Sepsis_parallel import (
...
if not PHASE1_IMPORTED:
    ...
    def _age_bracket(age) -> str:
        """Bin raw patient age into standard clinical age groups."""
        try:
            a = float(age)
    ...
    @dataclass
    class CaseInfo:
        """Information about a single Sepsis ICU patient case."""
        case_id: str
        outcome: int              # 1 = Return ER (Deviant), 0 = No Return ER (Normal)
        trace: List[str]          # Activity sequence with 'Return ER' stripped
        org_group: str            # Hospital department (org:group of admission event)
        age_bracket: str          # Binned patient age: <40 / 40-59 / 60-74 / 75+
    ...
    def extract_subgroups_from_case_data(case_data):
        """Sepsis subgroups: OrgGroup × AgeBracket (matches p1_Sepsis.py)."""
        case_to_subgroups = {}; subgroup_to_cases = {}
        for cid, case in case_data.items():
            sgs = []
            if case.org_group not in ('UNKNOWN', '', 'nan', 'None'):
                sg = f"OrgGroup_{case.org_group.replace(' ', '_')}"
                sgs.append(sg); subgroup_to_cases.setdefault(sg, set()).add(cid)
            if case.age_bracket not in ('UNKNOWN', '', 'nan', 'None'):
                sg = f"Age_{case.age_bracket}"
                sgs.append(sg); subgroup_to_cases.setdefault(sg, set()).add(cid)
...
def load_event_log(filepath: str) -> Dict[str, CaseInfo]:
    """
    Load Sepsis ICU event log and extract case information.

    Column mapping:
        case:concept:name  → case identifier
        time:timestamp     → event timestamp
        concept:name       → activity name (16 unique activities)
        org:group          → medical department (admission event)
        Age                → patient age (case-level, first event row)

    Outcome determination (Mannhardt 2016 / Teinemaa et al. TKDE 2019 sepsis_cases_1):
        Class 1 (Return ER / Deviant)   — 'Return ER' present anywhere in trace.
        Class 0 (No Return ER / Normal) — 'Return ER' absent from trace.

    Outcome-signal removal: 'Return ER' is stripped from the trace AFTER labelling.
    Retaining it would produce trivially dominant constraints (100% Class 1 / 0%
    Class 0) that encode the label rather than genuine process behaviour.
    """
    print(f"\n{'='*100}")
    ...
    # Drop 24 metadata rows with no case assignment (XES export artefact)
    ...
    for case_id, group in df.groupby('case:concept:name'):
        ...
        # Label: 1 if 'Return ER' appears anywhere, 0 otherwise
        outcome = 1 if RETURN_ER in all_activities else 0

        # Strip outcome signal to prevent trivially dominant patterns
        trace = [a for a in all_activities if a != RETURN_ER]
        if len(trace) == 0:
            continue

        activity_index = precompute_activity_index(trace, case_id=str(case_id))

        org_group_raw = case_events['org:group'].iloc[0]
        org_group = str(org_group_raw) if not pd.isna(org_group_raw) else 'UNKNOWN'

        age_raw = case_events['Age'].iloc[0] if 'Age' in case_events.columns else float('nan')
        age_bracket = _age_bracket(age_raw)
    ...
    print(f"   Class 1 (Return ER / Deviant):   {n1:,} ({n1/len(case_data)*100:.1f}%)")
    print(f"   Class 0 (No Return ER / Normal): {n0:,} ({n0/len(case_data)*100:.1f}%)")
...
def apply_P2_class_label_noise(
    ...
    Sepsis note: the Sepsis log has imbalance ratio ~3.9:1 (No-Return ER vs
    Return ER). P2 at ε=0.10 flips ~105 cases; at ε=0.20 flips ~210 of 1050.
    Because Class 1 is the minority, label flipping disproportionately
    dilutes Class 1 patterns faster than in balanced logs.
    """
    rng = np.random.RandomState(random_state)
...
def apply_P3_trace_truncation(
    ...
    Sepsis note: truncation simulates incomplete clinical documentation —
    a realistic failure mode in ICU/emergency settings where data entry
    stops before discharge. The Sepsis log has short traces (mean ~14
    events post-stripping); at ε=0.20 the median trace shrinks to ~11
    events, pushing Succession/ChainSuccession into high vacuity.
    Init/End constraints are largely immune (position-based, always
    applicable to any non-empty trace).
    """
    perturbed = {}
...
def run_phase1_on_perturbed_log(
    ...
    if not PHASE1_IMPORTED:
        raise RuntimeError(
            "run_phase1_on_perturbed_log requires the Phase 1 module (p1_Sepsis). "
...
def generate_rq3_visualizations(
    ...
    summary = f"""
    RQ3 DIRECTION 2 — SENSITIVITY ANALYSIS SUMMARY  [Sepsis ICU Log]
    ════════════════════════════════════════════════════════════════════════
    Framework: Sommers et al. (2025) Model-Transformation Perturbation
    Pipeline:  Dual-Axis Storey FDR (v8.0-DUAL-AXIS-STOREY-FDR)
    Log:       Sepsis Cases (Mannhardt 2016) — 1,050 cases, 15,190 events
    Outcome:   Return ER (deviant) vs. No Return ER (normal) — IR ≈ 3.9:1
...
{inf_text}
    Expected Degradation Patterns (Sepsis-specific):
      P1 (activity noise):  Structural axis degrades; ChainSuccession most fragile
                             due to short Sepsis traces (~14 events); discriminative unaffected
      P2 (class noise):     Discriminative axis degrades; inflection earlier (~0.05–0.10)
                             vs. balanced logs due to minority class dilution (IR=3.9)
      P3 (truncation):      Both axes degrade; vacuity rises rapidly for Succession family;
                             Init/End immune; simulates incomplete clinical documentation
    """
...
def save_rq3_outputs(
    ...
    rpt.append("=" * 120)
    rpt.append("SCIENTIFIC INTERPRETATION — SEPSIS ICU LOG")
    rpt.append("=" * 120)
    rpt.append("")
    rpt.append("Dataset: Sepsis Cases (Mannhardt 2016, 4TU doi:10.4121/uuid:915d2bfb)")
    rpt.append("Outcome: Return ER (deviant, Class 1) vs. No Return ER (normal, Class 0)")
    rpt.append("Imbalance ratio: ~3.9:1 (majority=No-Return ER; minority=Return ER)")
    rpt.append("Trace length post-stripping: mean ~14, median ~13 events")
    rpt.append("")
    rpt.append("Expected differential degradation patterns (Sepsis-specific):")
    rpt.append("")
    rpt.append("P1 (Activity label noise — targets H₀ˢ):")
    rpt.append("  - Structural axis degrades monotonically in ε")
    rpt.append("  - Short Sepsis traces amplify P1 sensitivity: a single swap in a 5-event")
    rpt.append("    trace disrupts ChainSuccession/ChainResponse far more than in long logs")
    ...
    rpt.append("    (earlier than balanced logs due to minority class dilution at IR=3.9)")
    rpt.append("  - At ε=0.10: ~105/1050 cases flipped; minority class (Return ER, ~212)")
    rpt.append("    loses ~50% of its genuine signal — Δ_obs collapses rapidly")
    ...
    rpt.append("  - Simulates incomplete clinical documentation (realistic Sepsis failure mode)")
    ...
    rpt.append("  - At ε=0.20: mean trace length ~11 events — binary consequents become")
    rpt.append("    vacuous for ~30% of cases; q-values inflate across both axes")
    rpt.append("")
    rpt.append("=" * 120)
