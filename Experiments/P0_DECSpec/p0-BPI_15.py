...
to two class-conditional sublogs derived from the outcome label:
    ...
    L+  =  { σ ∈ L  |  outcome(σ) = Not-Granted (Deviant) }
    L−  =  { σ ∈ L  |  outcome(σ) = Granted (Normal) }
...
independently for L+ and L−, producing:
    • phase0_declare_specification_Lpos.decl   — Not-Granted (Deviant) behaviour
    • phase0_declare_specification_Lneg.decl   — Granted (Normal) behaviour
...
OUTPUT_DIR = "../Experiments data/Experiments/Results/DECspec_BPI15"
...
def load_event_log(
    ...
    """Load BPI Challenge 2015 event log, extract activity traces, and partition
    into class sublogs using the case-level permit decision attribute.

    BPI Challenge 2015 column mapping
    ----------------------------------
        case:concept:name    → case identifier
        concept:name         → activity name
        time:timestamp       → event timestamp
        lifecycle:transition → event lifecycle (retain COMPLETE only)
        case:caseStatus      → case-level outcome attribute
                               'G'       → Granted  (Normal)  → L−
                               'O', 'T'  → Not-Granted (Deviant) → L+

    Lifecycle filter: only events with lifecycle:transition == 'COMPLETE' are
    retained.  BPI 2015 logs record START and COMPLETE events for each activity;
    using only COMPLETE preserves the canonical end-of-activity sequence and
    avoids artificially doubling each occurrence in the trace.

    Outcome labelling (van Dongen 2015 — BPI Challenge 2015)
    ---------------------------------------------------------
        case:caseStatus == 'G'  → outcome = 0 (Normal)   → L−
        case:caseStatus != 'G'  → outcome = 1 (Deviant)  → L+

    No outcome-signal removal is needed: the label is a case-level attribute,
    not an activity embedded in the event sequence.

    Returns
    -------
    cases     : full log L  (all valid cases)
    cases_pos : L+  =  { σ ∈ L  |  outcome(σ) = Not-Granted (Deviant) }
    cases_neg : L−  =  { σ ∈ L  |  outcome(σ) = Granted (Normal) }
    ...
    print(f"  Total events (raw):  {len(df):,}")
    print(f"  Total cases  (raw):  {df['case:concept:name'].nunique():,}")

    # Retain only COMPLETE lifecycle events (standard for BPI 2015)
    if 'lifecycle:transition' in df.columns:
        df = df[df['lifecycle:transition'].str.upper() == 'COMPLETE']
        print(f"  Events after COMPLETE filter: {len(df):,}")

    # Build case-level status lookup from the first non-null value per case
    case_status: Dict = (
        df.dropna(subset=['case:caseStatus'])
          .groupby('case:concept:name')['case:caseStatus']
          .first()
          .to_dict()
    )
    ...
    stats = {
        'total_cases': 0,
        'skipped_empty': 0,
        'class_0': 0,   # L−  (Granted / Normal)
        'class_1': 0,   # L+  (Not-Granted / Deviant)
    }

    for case_id, group in df.groupby('case:concept:name'):
        case_events = group.sort_values('time:timestamp')
        activities = case_events['activityNameEN'].tolist()

        if len(activities) == 0:
            stats['skipped_empty'] += 1
            continue

        # Outcome label derived from case-level attribute (not from the trace itself)
        status = case_status.get(case_id, None)
        outcome = 0 if status == 'G' else 1   # 0=Granted/Normal, 1=Not-Granted/Deviant
    ...
    cases_pos = [c for c in cases if c.outcome == 1]   # L+  (Not-Granted / Deviant)
    cases_neg = [c for c in cases if c.outcome == 0]   # L−  (Granted / Normal)

    print(f"\n  Processed cases: {stats['total_cases']:,}")
    print(f"  L+ | Class 1 (Not-Granted / Deviant):  {stats['class_1']:,} traces")
    print(f"  L− | Class 0 (Granted / Normal):       {stats['class_0']:,} traces")
    print(f"  Skipped (empty traces): {stats['skipped_empty']:,}")
    print(f"\n  Sublog partitioning:")
    print(f"    L  = {len(cases):,} traces")
    print(f"    L+ = {len(cases_pos):,} traces  (outcome = Not-Granted / Deviant)")
    print(f"    L− = {len(cases_neg):,} traces  (outcome = Granted / Normal)")
...
def generate_output(
    ...
    Outputs
    -------
    phase0_declare_specification_Lpos.decl  — L+ (Not-Granted / Deviant) specification
    phase0_declare_specification_Lneg.decl  — L− (Granted / Normal) specification
    phase0_declare_specification_CC.json    — enriched output:
        ...
        • set-algebra summary:
          ...
          These sets characterise what distinguishes Not-Granted from Granted
          process behaviour.
    """
    print("\n" + "=" * 100)
    ...
    print(f"\n  L+ (Not-Granted / Deviant) specification: {len(discovered_pos):,} constraints")
    for ct in ALL_CONSTRAINT_TYPES:
        if ct in by_type_pos:
            print(f"    {ct}: {by_type_pos[ct]}")

    print(f"\n  L− (Granted / Normal) specification: {len(discovered_neg):,} constraints")
    for ct in ALL_CONSTRAINT_TYPES:
    ...
    output = {
        ...
        'description': (
            ...
            'independently to L+ (Not-Granted / Deviant) and L− (Granted / Normal) sublogs. '
            'Constraint semantics follow Di Ciccio & Montali (2022) Table 2. '
            'Measure definitions follow Iacometta & Di Ciccio (2025) Table 2.'
        ),
        'references': [
        ...
        'dataset': {
            ...
            'class_Lpos_cases': stats['class_1'],   # Not-Granted (Deviant)
            'class_Lneg_cases': stats['class_0'],   # Granted (Normal)
        ...
        # --- L+ (Not-Granted / Deviant) specification ---
        'Lpos': {
            'label': 'L+ (Not-Granted / Deviant)',
        ...
        # --- L− (Granted / Normal) specification ---
        'Lneg': {
            'label': 'L− (Granted / Normal)',
        ...
        'set_algebra': {
            'description': (
                ...
                'Lpos_only: constraints exclusively characterising Not-Granted (Deviant) behaviour. '
                'Lneg_only: constraints exclusively characterising Granted (Normal) behaviour.'
            ),
    ...
    write_decl_file(
        ...
        decl_pos_path,
        header_comment=(
            f"Phase 0 Class-Conditional DECLARE Specification — L+ (Not-Granted / Deviant)\n"
            f"Generated: {ts}\n"
    ...
    write_decl_file(
        ...
        decl_neg_path,
        header_comment=(
            f"Phase 0 Class-Conditional DECLARE Specification — L− (Granted / Normal)\n"
            f"Generated: {ts}\n"
    ...
    # --- Console summaries ------------------------------------------------
    _print_top(discovered_pos, "L+ (Not-Granted / Deviant)")
    _print_top(discovered_neg, "L− (Granted / Normal)")

    if lpos_only:
        print(f"\n  L+-EXCLUSIVE constraints (Not-Granted-only behaviour, {len(lpos_only)}):")
        for c in lpos_only[:15]:
            m = c.measures
            print(f"    {c.pattern_id:<65} tconf={m.trace_confidence:.4f}")

    if lneg_only:
        print(f"\n  L−-EXCLUSIVE constraints (Granted-only behaviour, {len(lneg_only)}):")
...
def main():
    ...
    freq_pos = get_frequent_activities(
        cases_pos, CONFIG['min_activity_frequency'], class_label="L+ Not-Granted"
    )
    freq_neg = get_frequent_activities(
        cases_neg, CONFIG['min_activity_frequency'], class_label="L− Granted"
    )
    ...
    # -----------------------------------------------------------------------
    print("\n" + "=" * 100)
    print("DISCOVERY PIPELINE — L+ (Not-Granted / Deviant)")
    print("=" * 100)
    discovered_pos = discover_declare_specification(
        cases_pos, freq_pos, class_label="L+ Not-Granted"
    )

    print("\n" + "=" * 100)
    print("DISCOVERY PIPELINE — L− (Granted / Normal)")
    print("=" * 100)
    discovered_neg = discover_declare_specification(
        cases_neg, freq_neg, class_label="L− Granted"
    )
    ...
    print(f"  L+ (Not-Granted / Deviant) constraints: {len(discovered_pos)}")
    print(f"  L− (Granted / Normal)      constraints: {len(discovered_neg)}")
