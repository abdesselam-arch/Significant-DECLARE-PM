...
to two class-conditional sublogs derived from the outcome label:
    ...
    L+  =  { σ ∈ L  |  outcome(σ) = Return ER (Deviant) }
    L−  =  { σ ∈ L  |  outcome(σ) = No Return ER (Normal) }
...
independently for L+ and L−, producing:
    • phase0_declare_specification_Lpos.decl   — Return ER (Deviant) behaviour
    • phase0_declare_specification_Lneg.decl   — No Return ER (Normal) behaviour
...
OUTPUT_DIR = "../Experiments data/Experiments/Results/DECspec_Sepsis"
...
def load_event_log(
    ...
    """Load Sepsis ICU event log, extract full activity traces, and partition
    into class sublogs following Mannhardt (2016) and Teinemaa et al. (TKDE 2019).

    Sepsis column mapping
    ---------------------
        case:concept:name    → case identifier
        concept:name         → activity name  (16 clinical activities)
        time:timestamp       → event timestamp

    No lifecycle filter: the Sepsis log records each clinical activity as a
    single event (no start/complete split), so all events are retained.

    Outcome labelling (Mannhardt 2016 — sepsis_cases_1 benchmark)
    -------------------------------------------------------------
        'Return ER' present anywhere in trace  → outcome = 1 (Deviant)  → L+
        'Return ER' absent from trace          → outcome = 0 (Normal)   → L−

    Outcome-signal removal: 'Return ER' is stripped from the activity trace
    BEFORE building the DECLARE alphabet.  This prevents trivially vacuous
    constraints such as End[Return ER] holding in 100 % of L+ and 0 % of L−,
    which carry no genuine process-behavioural content.

    Returns
    -------
    cases     : full log L  (all valid cases)
    cases_pos : L+  =  { σ ∈ L  |  outcome(σ) = Return ER (Deviant) }
    cases_neg : L−  =  { σ ∈ L  |  outcome(σ) = No Return ER (Normal) }
    stats     : logging/audit dictionary
    """
    # Outcome signal — stripped from traces after labelling.
    RETURN_ER: str = 'Return ER'
    ...
    stats = {
        'total_cases': 0,
        'skipped_empty': 0,
        'class_0': 0,   # L−  (No Return ER / Normal)
        'class_1': 0,   # L+  (Return ER / Deviant)
    }

    for case_id, group in df.groupby('case:concept:name'):
        # All events sorted by timestamp — no lifecycle filter for Sepsis
    ...
    cases_pos = [c for c in cases if c.outcome == 1]   # L+  (Return ER / Deviant)
    cases_neg = [c for c in cases if c.outcome == 0]   # L−  (No Return ER / Normal)

    print(f"\n  Processed cases: {stats['total_cases']:,}")
    print(f"  L+ | Class 1 (Return ER / Deviant):   {stats['class_1']:,} traces")
    print(f"  L− | Class 0 (No Return ER / Normal): {stats['class_0']:,} traces")
    print(f"  Skipped (empty after signal removal): {stats['skipped_empty']:,}")
    print(f"\n  Sublog partitioning:")
    print(f"    L  = {len(cases):,} traces")
    print(f"    L+ = {len(cases_pos):,} traces  (outcome = Return ER / Deviant)")
    print(f"    L− = {len(cases_neg):,} traces  (outcome = No Return ER / Normal)")
...
def generate_output(
    ...
    Outputs
    -------
    phase0_declare_specification_Lpos.decl  — L+ (Return ER / Deviant) specification
    phase0_declare_specification_Lneg.decl  — L− (No Return ER / Normal) specification
    phase0_declare_specification_CC.json    — enriched output:
        ...
        • set-algebra summary:
          ...
          These sets characterise what distinguishes Return ER from No Return ER
          process behaviour.
    """
    print("\n" + "=" * 100)
    ...
    print(f"\n  L+ (Return ER / Deviant) specification: {len(discovered_pos):,} constraints")
    for ct in ALL_CONSTRAINT_TYPES:
        if ct in by_type_pos:
            print(f"    {ct}: {by_type_pos[ct]}")

    print(f"\n  L− (No Return ER / Normal)      specification: {len(discovered_neg):,} constraints")
    for ct in ALL_CONSTRAINT_TYPES:
    ...
    output = {
        ...
        'description': (
            ...
            'independently to L+ (Return ER / Deviant) and L− (No Return ER / Normal) sublogs. '
            'Constraint semantics follow Di Ciccio & Montali (2022) Table 2. '
            'Measure definitions follow Iacometta & Di Ciccio (2025) Table 2.'
        ),
        'references': [
        ...
        'dataset': {
            ...
            'class_Lpos_cases': stats['class_1'],   # Return ER (Deviant)
            'class_Lneg_cases': stats['class_0'],   # No Return ER (Normal)
        ...
        # --- L+ (Return ER / Deviant) specification ---
        'Lpos': {
            'label': 'L+ (Return ER / Deviant)',
        ...
        # --- L− (No Return ER / Normal) specification ---
        'Lneg': {
            'label': 'L− (No Return ER / Normal)',
        ...
        'set_algebra': {
            'description': (
                ...
                'Lpos_only: constraints exclusively characterising Return ER (Deviant) behaviour. '
                'Lneg_only: constraints exclusively characterising No Return ER (Normal) behaviour.'
            ),
    ...
    write_decl_file(
        ...
        decl_pos_path,
        header_comment=(
            f"Phase 0 Class-Conditional DECLARE Specification — L+ (Return ER / Deviant)\n"
            f"Generated: {ts}\n"
    ...
    write_decl_file(
        ...
        decl_neg_path,
        header_comment=(
            f"Phase 0 Class-Conditional DECLARE Specification — L− (No Return ER / Normal)\n"
            f"Generated: {ts}\n"
    ...
    # --- Console summaries ------------------------------------------------
    _print_top(discovered_pos, "L+ (Return ER / Deviant)")
    _print_top(discovered_neg, "L− (No Return ER / Normal)")

    if lpos_only:
        print(f"\n  L+-EXCLUSIVE constraints (Return ER-only behaviour, {len(lpos_only)}):")
        for c in lpos_only[:15]:
            m = c.measures
            print(f"    {c.pattern_id:<65} tconf={m.trace_confidence:.4f}")

    if lneg_only:
        print(f"\n  L−-EXCLUSIVE constraints (No Return ER-only behaviour, {len(lneg_only)}):")
...
def main():
    ...
    freq_pos = get_frequent_activities(
        cases_pos, CONFIG['min_activity_frequency'], class_label="L+ Return-ER"
    )
    freq_neg = get_frequent_activities(
        cases_neg, CONFIG['min_activity_frequency'], class_label="L− No-Return-ER"
    )
    ...
    # -----------------------------------------------------------------------
    print("\n" + "=" * 100)
    print("DISCOVERY PIPELINE — L+ (Return ER / Deviant)")
    print("=" * 100)
    discovered_pos = discover_declare_specification(
        cases_pos, freq_pos, class_label="L+ Return-ER"
    )

    print("\n" + "=" * 100)
    print("DISCOVERY PIPELINE — L− (No Return ER / Normal)")
    print("=" * 100)
    discovered_neg = discover_declare_specification(
        cases_neg, freq_neg, class_label="L− No-Return-ER"
    )
    ...
    print(f"  L+ (Return ER / Deviant) constraints: {len(discovered_pos)}")
    print(f"  L− (No Return ER / Normal)      constraints: {len(discovered_neg)}")
