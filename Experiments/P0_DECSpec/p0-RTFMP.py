...
to two class-conditional sublogs derived from the outcome label:
    ...
    L+  =  { σ ∈ L  |  outcome(σ) = Sent for Credit Collection (Deviant) }
    L−  =  { σ ∈ L  |  outcome(σ) = No Credit Collection (Normal) }
...
independently for L+ and L−, producing:
    • phase0_declare_specification_Lpos.decl   — Sent for Credit Collection (Deviant) behaviour
    • phase0_declare_specification_Lneg.decl   — No Credit Collection (Normal) behaviour
...
INPUT_FILE = "../Experiments data/CSV/Road_Traffic_Fine_Management_Process.csv"
OUTPUT_DIR = "../Experiments data/Experiments/Results/DECspec_RTFMP"
...
def load_event_log(
    ...
    """Load RTFMP event log, extract full activity traces, and partition
    into class sublogs following Teinemaa et al. (TKDE 2019).

    RTFMP column mapping
    --------------------
    ...
    Lifecycle filter: the RTFMP log records both 'start' and 'complete' events
    per activity.  Only 'complete' rows are retained before building traces;
    retaining starts would duplicate every activity in every trace, inflating
    trace lengths and distorting all DECLARE pattern prevalence estimates.

    Outcome labelling (Teinemaa et al. TKDE 2019 — canonical RTFMP binary outcome)
    -------------------------------------------------------------------------------
        'Send for Credit Collection' present anywhere in trace → outcome = 1 (Deviant) → L+
        'Send for Credit Collection' absent from trace         → outcome = 0 (Normal)  → L−

    Outcome-signal removal: 'Send for Credit Collection' is stripped from the
    activity trace BEFORE building the DECLARE alphabet.  This prevents trivially
    vacuous constraints such as End[Send for Credit Collection] holding in 100 % of
    L+ and 0 % of L−, which carry no genuine process-behavioural content.

    Returns
    -------
    cases     : full log L  (all valid cases)
    cases_pos : L+  =  { σ ∈ L  |  outcome(σ) = Sent for Credit Collection (Deviant) }
    cases_neg : L−  =  { σ ∈ L  |  outcome(σ) = No Credit Collection (Normal) }
    stats     : logging/audit dictionary
    """
    # Outcome signal — stripped from traces after labelling.
    TARGET_ACTIVITY: str = 'Send for Credit Collection'
    ...
    stats = {
        'total_cases': 0,
        'skipped_empty': 0,
        'class_0': 0,   # L−  (No Credit Collection / Normal)
        'class_1': 0,   # L+  (Sent for Credit Collection / Deviant)
    }
    ...
    for case_id, group in df.groupby('case:concept:name'):
        ...
        # Label: 1 if target activity appears anywhere in the (complete) trace
        # Convention: Teinemaa et al. (TKDE 2019)
        ...
        activities = [a for a in all_activities if a != TARGET_ACTIVITY]
    ...
    cases_pos = [c for c in cases if c.outcome == 1]   # L+  (Sent for Credit Collection / Deviant)
    cases_neg = [c for c in cases if c.outcome == 0]   # L−  (No Credit Collection / Normal)

    print(f"\n  Processed cases: {stats['total_cases']:,}")
    print(f"  L+ | Class 1 (Sent for Credit Collection / Deviant): {stats['class_1']:,} traces")
    print(f"  L− | Class 0 (No Credit Collection / Normal):        {stats['class_0']:,} traces")
    print(f"  Skipped (empty after signal removal): {stats['skipped_empty']:,}")
    print(f"\n  Sublog partitioning:")
    print(f"    L  = {len(cases):,} traces")
    print(f"    L+ = {len(cases_pos):,} traces  (outcome = Sent for Credit Collection / Deviant)")
    print(f"    L− = {len(cases_neg):,} traces  (outcome = No Credit Collection / Normal)")
...
def generate_output(
    ...
    Outputs
    -------
    phase0_declare_specification_Lpos.decl  — L+ (Sent for Credit Collection / Deviant) specification
    phase0_declare_specification_Lneg.decl  — L− (No Credit Collection / Normal) specification
    phase0_declare_specification_CC.json    — enriched output:
        ...
        • set-algebra summary:
          ...
          These sets characterise what distinguishes Sent for Credit Collection
          from No Credit Collection process behaviour.
    """
    print("\n" + "=" * 100)
    ...
    print(f"\n  L+ (Sent for Credit Collection / Deviant) specification: {len(discovered_pos):,} constraints")
    for ct in ALL_CONSTRAINT_TYPES:
        if ct in by_type_pos:
            print(f"    {ct}: {by_type_pos[ct]}")

    print(f"\n  L− (No Credit Collection / Normal) specification: {len(discovered_neg):,} constraints")
    for ct in ALL_CONSTRAINT_TYPES:
    ...
    output = {
        ...
        'description': (
            ...
            'independently to L+ (Sent for Credit Collection / Deviant) and L− (No Credit Collection / Normal) sublogs. '
            'Constraint semantics follow Di Ciccio & Montali (2022) Table 2. '
            'Measure definitions follow Iacometta & Di Ciccio (2025) Table 2.'
        ),
        'references': [
        ...
        'dataset': {
            ...
            'class_Lpos_cases': stats['class_1'],   # Sent for Credit Collection (Deviant)
            'class_Lneg_cases': stats['class_0'],   # No Credit Collection (Normal)
        ...
        # --- L+ (Sent for Credit Collection / Deviant) specification ---
        'Lpos': {
            'label': 'L+ (Sent for Credit Collection / Deviant)',
        ...
        # --- L− (No Credit Collection / Normal) specification ---
        'Lneg': {
            'label': 'L− (No Credit Collection / Normal)',
        ...
        'set_algebra': {
            'description': (
                ...
                'Lpos_only: constraints exclusively characterising Sent for Credit Collection (Deviant) behaviour. '
                'Lneg_only: constraints exclusively characterising No Credit Collection (Normal) behaviour.'
            ),
    ...
    write_decl_file(
        ...
        decl_pos_path,
        header_comment=(
            f"Phase 0 Class-Conditional DECLARE Specification — L+ (Sent for Credit Collection / Deviant)\n"
            f"Generated: {ts}\n"
    ...
    write_decl_file(
        ...
        decl_neg_path,
        header_comment=(
            f"Phase 0 Class-Conditional DECLARE Specification — L− (No Credit Collection / Normal)\n"
            f"Generated: {ts}\n"
    ...
    # --- Console summaries ------------------------------------------------
    _print_top(discovered_pos, "L+ (Sent for Credit Collection / Deviant)")
    _print_top(discovered_neg, "L− (No Credit Collection / Normal)")

    if lpos_only:
        print(f"\n  L+-EXCLUSIVE constraints (Credit Collection-only behaviour, {len(lpos_only)}):")
        for c in lpos_only[:15]:
            m = c.measures
            print(f"    {c.pattern_id:<65} tconf={m.trace_confidence:.4f}")

    if lneg_only:
        print(f"\n  L−-EXCLUSIVE constraints (No Credit Collection-only behaviour, {len(lneg_only)}):")
...
def main():
    ...
    freq_pos = get_frequent_activities(
        cases_pos, CONFIG['min_activity_frequency'], class_label="L+ Credit-Collection"
    )
    freq_neg = get_frequent_activities(
        cases_neg, CONFIG['min_activity_frequency'], class_label="L− No-Credit-Collection"
    )
    ...
    # -----------------------------------------------------------------------
    print("\n" + "=" * 100)
    print("DISCOVERY PIPELINE — L+ (Sent for Credit Collection / Deviant)")
    print("=" * 100)
    discovered_pos = discover_declare_specification(
        cases_pos, freq_pos, class_label="L+ Credit-Collection"
    )

    print("\n" + "=" * 100)
    print("DISCOVERY PIPELINE — L− (No Credit Collection / Normal)")
    print("=" * 100)
    discovered_neg = discover_declare_specification(
        cases_neg, freq_neg, class_label="L− No-Credit-Collection"
    )
    ...
    print(f"  L+ (Sent for Credit Collection / Deviant) constraints: {len(discovered_pos)}")
    print(f"  L− (No Credit Collection / Normal)        constraints: {len(discovered_neg)}")
