...
to two class-conditional sublogs derived from the outcome label:
    ...
    L+  =  { σ ∈ L  |  outcome(σ) = Deviant (Re-submission Required) }
    L−  =  { σ ∈ L  |  outcome(σ) = Regular (Normal — No Re-submission) }
...
independently for L+ and L−, producing:
    • phase0_declare_specification_Lpos.decl   — Deviant (Re-submission Required) behaviour
    • phase0_declare_specification_Lneg.decl   — Regular (Normal — No Re-submission) behaviour
...
OUTPUT_DIR = "../Experiments data/Experiments/Results/DECspec_Production"
...
def load_event_log(
    ...
    """Load Production event log, extract full activity traces, and partition
    into class sublogs.
    ...
    No lifecycle filter: each row in the Production log represents a single
    completed event (no start/complete split); all events are retained.

    Outcome labelling (Teinemaa et al. TKDE 2019 — Production benchmark)
    ---------------------------------------------------------------------
    The outcome label is pre-encoded in the 'label' column:
        'deviant'   → outcome = 1  → L+  (work order required re-submission)
        'regular'   → outcome = 0  → L−  (work order completed without re-submission)

    No outcome-signal removal: unlike signal-derived logs (e.g. Sepsis,
    RTFMP), the Production label is stored in a dedicated attribute column and
    is not expressed as an activity event in the trace.  All activity events
    therefore carry genuine process-behavioural content and are retained in full.

    Returns
    -------
    cases     : full log L  (all valid labelled cases)
    cases_pos : L+  =  { σ ∈ L  |  outcome(σ) = Deviant (Re-submission Required) }
    cases_neg : L−  =  { σ ∈ L  |  outcome(σ) = Regular (Normal — No Re-submission) }
    ...
    expected = {'regular', 'deviant'}
    unexpected = label_values - expected
    if unexpected:
        print(f"  WARNING: unexpected label values found: {unexpected}")

    cases: List[CaseTrace] = []
    stats = {
        'total_cases': 0,
        'skipped_empty': 0,
        'class_0': 0,   # L−  (Regular / Normal — No Re-submission)
        'class_1': 0,   # L+  (Deviant / Re-submission Required)
    }

    for case_id, group in df.groupby('Case ID'):
        # Events sorted by completion timestamp
        case_events = group.sort_values('Complete Timestamp')
        activities = case_events['Activity'].dropna().tolist()

        if len(activities) == 0:
            stats['skipped_empty'] += 1
            continue

        # Outcome from the pre-encoded label column.
        # The label is case-level: all rows for a case share the same value,
        # so the first non-null entry is sufficient.
    ...
    cases_pos = [c for c in cases if c.outcome == 1]   # L+  (Deviant / Re-submission Required)
    cases_neg = [c for c in cases if c.outcome == 0]   # L−  (Regular / Normal — No Re-submission)

    print(f"\n  Processed cases: {stats['total_cases']:,}")
    print(f"  L+ | Class 1 (Deviant — Re-submission Required): {stats['class_1']:,} traces")
    print(f"  L− | Class 0 (Regular — No Re-submission):       {stats['class_0']:,} traces")
    print(f"  Skipped (empty trace):                           {stats['skipped_empty']:,}")
    print(f"\n  Sublog partitioning:")
    print(f"    L  = {len(cases):,} traces")
    print(f"    L+ = {len(cases_pos):,} traces  (outcome = Deviant — Re-submission Required)")
    print(f"    L− = {len(cases_neg):,} traces  (outcome = Regular — No Re-submission)")
...
def generate_output(
    ...
    Outputs
    -------
    phase0_declare_specification_Lpos.decl  — L+ (Deviant — Re-submission Required) specification
    phase0_declare_specification_Lneg.decl  — L− (Regular — No Re-submission) specification
    phase0_declare_specification_CC.json    — enriched output:
        ...
        • set-algebra summary:
          ...
          These sets characterise what distinguishes Deviant from Regular
    ...
    print(f"\n  L+ (Deviant — Re-submission Required) specification: {len(discovered_pos):,} constraints")
    for ct in ALL_CONSTRAINT_TYPES:
        if ct in by_type_pos:
            print(f"    {ct}: {by_type_pos[ct]}")

    print(f"\n  L− (Regular — No Re-submission) specification: {len(discovered_neg):,} constraints")
    for ct in ALL_CONSTRAINT_TYPES:
    ...
    output = {
        ...
        'description': (
            ...
            'independently to L+ (Deviant — Re-submission Required) and L− (Regular — No Re-submission) sublogs. '
            'Constraint semantics follow Di Ciccio & Montali (2022) Table 2. '
            'Measure definitions follow Iacometta & Di Ciccio (2025) Table 2.'
        ),
        'references': [
        ...
        'dataset': {
            ...
            'class_Lpos_cases': stats['class_1'],   # Deviant (Re-submission Required)
            'class_Lneg_cases': stats['class_0'],   # Regular (Normal — No Re-submission)
        ...
        # --- L+ (Deviant — Re-submission Required) specification ---
        'Lpos': {
            'label': 'L+ (Deviant — Re-submission Required)',
        ...
        # --- L− (Regular — No Re-submission) specification ---
        'Lneg': {
            'label': 'L− (Regular — No Re-submission)',
        ...
        'set_algebra': {
            'description': (
                ...
                'Lpos_only: constraints exclusively characterising Deviant (Re-submission Required) behaviour. '
                'Lneg_only: constraints exclusively characterising Regular (Normal — No Re-submission) behaviour.'
            ),
    ...
    write_decl_file(
        ...
        decl_pos_path,
        header_comment=(
            f"Phase 0 Class-Conditional DECLARE Specification — L+ (Deviant — Re-submission Required)\n"
            f"Generated: {ts}\n"
    ...
    write_decl_file(
        ...
        decl_neg_path,
        header_comment=(
            f"Phase 0 Class-Conditional DECLARE Specification — L− (Regular — No Re-submission)\n"
            f"Generated: {ts}\n"
    ...
    # --- Console summaries ------------------------------------------------
    _print_top(discovered_pos, "L+ (Deviant — Re-submission Required)")
    _print_top(discovered_neg, "L− (Regular — No Re-submission)")

    if lpos_only:
        print(f"\n  L+-EXCLUSIVE constraints (Deviant-only behaviour, {len(lpos_only)}):")
        for c in lpos_only[:15]:
            m = c.measures
            print(f"    {c.pattern_id:<65} tconf={m.trace_confidence:.4f}")

    if lneg_only:
        print(f"\n  L−-EXCLUSIVE constraints (Regular-only behaviour, {len(lneg_only)}):")
...
def main():
    ...
    freq_pos = get_frequent_activities(
        cases_pos, CONFIG['min_activity_frequency'], class_label="L+ Deviant"
    )
    freq_neg = get_frequent_activities(
        cases_neg, CONFIG['min_activity_frequency'], class_label="L− Regular"
    )
    ...
    # -----------------------------------------------------------------------
    print("\n" + "=" * 100)
    print("DISCOVERY PIPELINE — L+ (Deviant — Re-submission Required)")
    print("=" * 100)
    discovered_pos = discover_declare_specification(
        cases_pos, freq_pos, class_label="L+ Deviant"
    )

    print("\n" + "=" * 100)
    print("DISCOVERY PIPELINE — L− (Regular — No Re-submission)")
    print("=" * 100)
    discovered_neg = discover_declare_specification(
        cases_neg, freq_neg, class_label="L− Regular"
    )
    ...
    print(f"  L+ (Deviant — Re-submission Required) constraints: {len(discovered_pos)}")
    print(f"  L− (Regular — No Re-submission)       constraints: {len(discovered_neg)}")
