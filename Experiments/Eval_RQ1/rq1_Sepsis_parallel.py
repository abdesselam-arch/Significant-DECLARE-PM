...
This is NOT an architectural change to p1_Sepsis.py.  The Phase 1 framework,
...
partition (pi0_struct ~ 0.46).  The Fisher statistic for those patterns remains
shifted rightward:
...
Version : 3.1  (empirical Phipson-Smyth calibration for Storey gate; aligned with p1_Sepsis.py v8.0)
...
def run_original_pipeline(n_workers: int = 1) -> dict:
    """
    Run p1_Sepsis.py's execute_pipeline with full computational budget.
...
def run_doubly_null_replicate(
    ...
    This faithfully reproduces p1_Sepsis.py's decision procedure on a log
    ...
    p1_Sepsis.py Step 5b uses Phipson-Smyth empirically-calibrated p-values
...
def main():
    ...
    print("  Aligned with p1_Sepsis.py v8.0 (empirical Phipson-Smyth Storey gate)")
    print("=" * 100)
    ...
    print(f"\n  Decision procedure (faithful to p1_Sepsis.py):")
