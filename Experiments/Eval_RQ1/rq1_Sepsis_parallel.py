#!/usr/bin/env python3
"""
rq1_Sepsis_parallel.py  —  RQ1 FDR Control Validity: Sepsis ICU
================================================================
...
-----------------------------------------------
    1.  P1  (Ours)       Hou-Storey conjunction + Adaptive Storey FDR gate.
                         ...
                         Aligned with p1_Sepsis_parallel.py v9.0-HOU-DOUBLY-NULL.
    ...
    2.  DRVA             Cecconi et al. (BPM Forum 2021).
                         ...
                         Imported from drva_Sepsis_parallel.py.
    ...
    3.  DeclareMiner     Differential confidence threshold baseline.
                         ...
                         Imported from declareminer_Sepsis_parallel.py.
...
Version : 1.0  (Hou-Storey + DRVA + DeclareMiner; single gate; aligned with
               p1_Sepsis_parallel.py v9.0-HOU-DOUBLY-NULL)
...
# File: p1_Sepsis_parallel.py  (v9.0-HOU-DOUBLY-NULL)
...
# ── DRVA baseline ─────────────────────────────────────────────────────────
# File: drva_Sepsis_parallel.py  (v2.1)
...
# File: declareminer_Sepsis_parallel.py  (v2.0)
...
# ── Permutation budgets ────────────────────────────────────────────────────
# Original P1 run: matches p1_Sepsis_parallel CONFIG exactly.
...
def run_p1_original(n_workers: int = 1) -> dict:
    ...
    The full budget matches p1_Sepsis_parallel CONFIG:
...
def run_drva_original(
    ...
    """
    Run DRVA on the real Sepsis data with the shared M_all candidate pool.
...
def compute_and_save_metrics(
    ...
    print(f"\n  FDR Results (Sepsis, B_null={B_NULL}, double-null protocol):")
    print(f"  {'─'*90}")
    ...
    full_json = {
        ...
        'alignment_with_p1': {
            ...
            'single_gate': (
                ...
                'Matches p1_Sepsis_parallel.py v9.0-HOU-DOUBLY-NULL. '
...
def main():
    print("\n" + "=" * 100)
    print("RQ1 — FDR CONTROL VALIDITY: SEPSIS ICU")
    ...
    print(f"\n{'='*100}")
    print("RQ1 — SEPSIS COMPLETE  (double-null, three-method comparison)")
    print(f"{'='*100}")
