#!/usr/bin/env python3
"""
rq1_BPI_15.py  —  RQ1 FDR Control Validity: BPI Challenge 2015
=============================================================================

PURPOSE
-------
Empirically validate that the Fisher-Storey FDR framework controls the
false discovery rate at nominal alpha = 0.05 on the BPI Challenge 2015
(municipality building permit) event log, following the gold-standard
held-out permutation protocol of Pellegrina & Vandin (KDD 2018).

Label definition (Teinemaa et al. TKDE 2019 — BPI 2015 binary outcome):
    Deviant (1): case:caseStatus != 'G'  -> permit not granted (objection/other)
    Normal  (0): case:caseStatus == 'G'  -> permit granted
...
This is NOT an architectural change to p1_BPI_15.py.  The Phase 1 framework,
...
In BPI 2015, many patterns reflecting the multi-phase permit procedure have
...
Version : 3.1  (empirical Phipson-Smyth calibration for Storey gate; aligned with p1_BPI_15.py v8.0)
...
from Experiments.P1_SDSM.p1_BPI_15 import (
...
RQ1_OUTPUT_DIR = "../Experiments data/Experiments/Results/RQ1_BPI15"
...
def run_original_pipeline() -> dict:
    """
    Run p1_BPI_15.py's execute_pipeline with full computational budget.
    Returns all objects needed for R_obs computation and RQ1 evaluation.
    """
    print("\n" + "=" * 100)
    print("SECTION 1 — BPI 2015: ORIGINAL-DATA RUN VIA execute_pipeline()")
...
def run_doubly_null_replicate(
    ...
    This faithfully reproduces p1_BPI_15.py's decision procedure on a log
    ...
    p1_BPI_15.py Step 5b uses Phipson-Smyth empirically-calibrated p-values
...
def compute_and_save_metrics(
    ...
    results_df = build_rq1_results_df(
        ...
        alpha=ALPHA, log_name="BPI15",
    )

    print(f"\n  FDR Validation Results (BPI 2015, B_null={B_NULL}, double-null protocol):")
    print(f"  {'─'*95}")
    ...
    full_json['null_protocol'] = {
        ...
        'why_label_only_fails': (
            ...
            'Succession(01_HOOFD intake, 02_BEZWAAR objection) in BPI 2015), structural p-values '
...
def main():
    print("\n" + "=" * 100)
    print("RQ1 — FDR CONTROL VALIDITY: BPI CHALLENGE 2015")
    print("  Double-Null Protocol: sigma_label ∘ sigma_trace")
    print("  Aligned with p1_BPI_15.py v8.0 (empirical Phipson-Smyth Storey gate)")
    print("=" * 100)
    ...
    print(f"\n  Decision procedure (faithful to p1_BPI_15.py):")
    ...
    print(f"\n{'='*100}")
    print("RQ1 — BPI 2015 COMPLETE (Double-Null Protocol)")
    print(f"{'='*100}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="RQ1 FDR Control Validity — BPI Challenge 2015 (Double-Null Protocol)"
    )
    parser.add_argument(
