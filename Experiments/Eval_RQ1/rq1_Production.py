#!/usr/bin/env python3
"""
rq1_Production.py  —  RQ1 FDR Control Validity: Production (Near-Balanced Small Log)
=====================================================================================

PURPOSE
-------
Empirically validate that the Hou-Storey FDR framework controls the
false discovery rate at nominal alpha = 0.05 on the Production event log
(manufacturing work orders), following the gold-standard held-out permutation
protocol of Pellegrina & Vandin (KDD 2018).

Label definition (pre-encoded case attribute in Production.csv, sep=';'):
    Deviant (1): 'deviant'  -> work order required at least one re-submission
    Normal  (0): 'regular'  -> work order completed without re-submission
    n=220 cases (117 deviant / 103 regular), IR ≈ 1.136 (near-balanced)
...
This is NOT an architectural change to p1_Production_hou.py.  The Phase 1 framework,
...
In Production, manufacturing work orders follow fixed process templates with strong
sequential regularity; structural p-values remain small under any random class
...
ALIGNMENT WITH p1_Production_hou.py v9.0
-----------------------------------------
...
execute_three_hypothesis_protocol. Key design decisions:
    ...
    1.  Score (Hou 2005 weighted):
            ...
            Production CONFIG: B_label=1500, B2_test=B_trace//2=1000 →
    ...
    7.  Single-gate significance (p1 Step 5c):
        ...
        This matches p1_Production_hou.py Step 5b: is_significant_final =
...
---------------------
    ...
    Production scale: n=220 cases (very small log — ~5x cheaper than Sepsis).
    Per-replicate cost (double-null):
        ~0.1 min  trace shuffling + holds recomputation
        ~0.5 min  label permutation test (B1=2000)
        ~1.0 min  structural permutation test (B2=500, both classes)
        ~2 min    total per replicate

    Total: 200 x 2 min / 8 cores ~ 1 HPC hour
...
Version : 5.0  (Hou-Storey, single gate, oracle analytic calibration — aligned with p1_Production_hou.py v9.0-HOU-DOUBLY-NULL)
...
from Experiments.P1_SDSM.p1_Production_hou import (
...
# RQ1_OUTPUT_DIR = "../Experiments data/Experiments/RQ1_Production"
RQ1_OUTPUT_DIR = "RQ1_Production"
...
def run_original_pipeline(n_workers: int = 1) -> dict:
    """
    Run p1_Production_hou.py's execute_pipeline with full computational budget.
    ...
    """
    print("\n" + "=" * 100)
    print("SECTION 1 — PRODUCTION: ORIGINAL-DATA RUN VIA execute_pipeline()")
    ...
    # ── ADD: write p1's results to Production_Results/ ───────────────────
    generate_outputs(
...
def run_doubly_null_replicate(
    ...
    This faithfully reproduces p1_Production_hou.py's decision procedure on a log
    ...
    # This matches p1_Production_hou.py: is_significant_final = is_significant_discriminative.
    if m_prime > 0:
        pi0_f, _ = adaptive_storey_pi0(p_hou_m_prime, q=alpha)
        q_hou = storey_qvalue(p_hou_m_prime, pi0_f)
...
def compute_and_save_metrics(
    ...
    results_df = build_rq1_results_df(
        ...
        alpha=ALPHA, log_name="Production",
    )

    print(f"\n  FDR Validation Results (Production, B_null={B_NULL}, double-null protocol):")
    print(f"  {'─'*95}")
    ...
    full_json['null_protocol'] = {
        ...
        'why_label_only_fails': (
            ...
            'Succession(machining, quality check) in Production), structural p-values '
    ...
    full_json['alignment_with_p1'] = {
        ...
        'significance_gate': (
            'SINGLE gate: q_Hou <= alpha only. '
            'Matches p1_Production_hou.py is_significant_final = is_significant_discriminative. '
            'Structural evidence is embedded in T_Hou via the W_STRUCT weight. '
    ...
    full_json['config'] = {
        ...
        'N_JOBS': N_JOBS,
        'log': 'Production', 'p1_version': 'p1_Production_hou.py v9.0-HOU-DOUBLY-NULL',
...
def main():
    print("\n" + "=" * 100)
    print("RQ1 — FDR CONTROL VALIDITY: PRODUCTION (Near-Balanced Small Log)")
    print("  Double-Null Protocol: sigma_label ∘ sigma_trace")
    print("  Aligned with p1_Production_hou.py v9.0 (Hou 2005 weighted conjunction, empirical Phipson-Smyth)")
    print("=" * 100)
    ...
    print(f"\n  Decision procedure (faithful to p1_Production_hou.py v9.0):")
    ...
    print(f"\n{'='*100}")
    print("RQ1 — PRODUCTION COMPLETE (Double-Null Protocol)")
    print(f"{'='*100}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="RQ1 FDR Control Validity — Production (Double-Null Protocol)"
    )
    parser.add_argument(
