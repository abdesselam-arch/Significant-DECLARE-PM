#!/usr/bin/env python3
"""
rq1_RTFMP.py  —  RQ1 FDR Control Validity: RTFMP (Mildly Imbalanced Large Log)
===============================================================================

PURPOSE
-------
Empirically validate that the Hou-Storey FDR framework controls the
false discovery rate at nominal alpha = 0.05 on the Road Traffic Fine
Management Process (RTFMP) event log, following the gold-standard held-out
permutation protocol of Pellegrina & Vandin (KDD 2018).

Label definition (Teinemaa et al. TKDE 2019 — canonical RTFM binary outcome):
    Deviant (1): case contains activity "Send for Credit Collection"
                 -> fine escalated to credit collection agency
    Normal  (0): "Send for Credit Collection" never occurs in trace
                 -> fine resolved without credit collection
    IR ≈ 1.548 (91,357 normal / 59,013 deviant out of 150,370 total cases)
...
This is NOT an architectural change to p1_RTFMP_hou.py.  The Phase 1 framework,
...
In RTFMP, the process is highly structured (316 unique variants over 150,370 cases),
so many patterns have temporal regularity that persists across any random class
partition.  The Hou weighted statistic for those patterns remains shifted rightward:
...
ALIGNMENT WITH p1_RTFMP_hou.py v9.0
-------------------------------------
...
execute_three_hypothesis_protocol. Key design decisions:
    ...
    1.  Score (Hou 2005 weighted):
            ...
            RTFMP CONFIG: B_label=1500, B2_test=B_trace//2=1000 →
    ...
    7.  Single-gate significance (p1 Step 5c):
        ...
        This matches p1_RTFMP_hou.py Step 5b: is_significant_final =
...
---------------------
    ...
    RTFMP scale: n=150,370 cases (vs ~1,050 for Sepsis).
    Holds recomputation is O(n·m) — dominant cost, ~143x more expensive than Sepsis.
    Per-replicate cost is wall-time-dependent; use --dry-run first to estimate.
    Label permutation test (B1=2,000) and structural test (B2=500) scale linearly in n.

    Total: TBD — run with --dry-run --b-null 2 to measure per-replicate wall time.
...
Version : 5.0  (Hou-Storey, single gate, oracle analytic calibration — aligned with p1_RTFMP_hou.py v9.0-HOU-DOUBLY-NULL)
...
from Experiments.P1_SDSM.p1_RTFMP_hou import (
...
# RQ1_OUTPUT_DIR = "../Experiments data/Experiments/RQ1_RTFMP"
RQ1_OUTPUT_DIR = "RQ1_RTFMP"
...
def run_original_pipeline(n_workers: int = 1) -> dict:
    """
    Run p1_RTFMP_hou.py's execute_pipeline with full computational budget.
    ...
    """
    print("\n" + "=" * 100)
    print("SECTION 1 — RTFMP: ORIGINAL-DATA RUN VIA execute_pipeline()")
    ...
    # ── ADD: write p1's results to RTFMP_Results/ ────────────────────────
    generate_outputs(
...
def run_doubly_null_replicate(
    ...
    This faithfully reproduces p1_RTFMP_hou.py's decision procedure on a log
    ...
    # This matches p1_RTFMP_hou.py: is_significant_final = is_significant_discriminative.
    if m_prime > 0:
        pi0_f, _ = adaptive_storey_pi0(p_hou_m_prime, q=alpha)
        q_hou = storey_qvalue(p_hou_m_prime, pi0_f)
...
def compute_and_save_metrics(
    ...
    results_df = build_rq1_results_df(
        ...
        alpha=ALPHA, log_name="RTFMP",
    )

    print(f"\n  FDR Validation Results (RTFMP, B_null={B_NULL}, double-null protocol):")
    print(f"  {'─'*95}")
    ...
    full_json['null_protocol'] = {
        ...
        'why_label_only_fails': (
            ...
            'Response(Create Fine, Send Fine) in RTFMP), structural p-values '
            'remain small in any random class partition. The Hou statistic '
    ...
    full_json['alignment_with_p1'] = {
        ...
        'significance_gate': (
            'SINGLE gate: q_Hou <= alpha only. '
            'Matches p1_RTFMP_hou.py is_significant_final = is_significant_discriminative. '
            'Structural evidence is embedded in T_Hou via the W_STRUCT weight. '
    ...
    full_json['config'] = {
        ...
        'N_JOBS': N_JOBS,
        'log': 'RTFMP', 'p1_version': 'p1_RTFMP_hou.py v9.0-HOU-DOUBLY-NULL',
...
def main():
    print("\n" + "=" * 100)
    print("RQ1 — FDR CONTROL VALIDITY: RTFMP (Mildly Imbalanced Large Log)")
    print("  Double-Null Protocol: sigma_label ∘ sigma_trace")
    print("  Aligned with p1_RTFMP_hou.py v9.0 (Hou 2005 weighted conjunction, empirical Phipson-Smyth)")
    print("=" * 100)
    ...
    print(f"\n  Decision procedure (faithful to p1_RTFMP_hou.py v9.0):")
    ...
    print(f"\n{'='*100}")
    print("RQ1 — RTFMP COMPLETE (Double-Null Protocol)")
    print(f"{'='*100}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="RQ1 FDR Control Validity — RTFMP (Double-Null Protocol)"
    )
    parser.add_argument(
