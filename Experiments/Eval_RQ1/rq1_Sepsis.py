#!/usr/bin/env python3
"""
rq1_Sepsis.py  —  RQ1 FDR Control Validity: Sepsis (Imbalanced Stress-Test)
=============================================================================

PURPOSE
-------
Empirically validate that the Hou-Storey FDR framework controls the
false discovery rate at nominal alpha = 0.05 on the Sepsis ICU event log,
following the gold-standard held-out permutation protocol of Pellegrina
& Vandin (KDD 2018).

Label definition (Mannhardt 2016, used in Teinemaa et al. TKDE 2019):
    Deviant (1): case contains activity "Return ER"  -> patient readmitted
    Normal  (0): no "Return ER" in trace             -> discharged w/o readmission
...
This is NOT an architectural change to p1_Sepsis_hou.py.  The Phase 1 framework,
...
In Sepsis, ~54% of patterns have structural signal visible in any random class
partition (pi0_struct ~ 0.46).  The Hou weighted statistic for those patterns remains
shifted rightward:
...
ALIGNMENT WITH p1_Sepsis_hou.py v9.0
--------------------------------------
...
execute_three_hypothesis_protocol. Key design decisions:
    ...
    1.  Score (Hou 2005 weighted):
            ...
            Sepsis CONFIG: B_label=1500, B2_test=B_trace//2=1000 →
    ...
    7.  Single-gate significance (p1 Step 5c):
        ...
        This matches p1_Sepsis_hou.py Step 5b: is_significant_final =
...
Version : 5.0  (Hou-Storey, single gate, oracle analytic calibration — aligned with p1_Sepsis_hou.py v9.0-HOU-DOUBLY-NULL)
...
from Experiments.P1_SDSM.p1_Sepsis_hou import (
...
# RQ1_OUTPUT_DIR = "../Experiments data/Experiments/RQ1_Sepsis"
...
def run_original_pipeline(n_workers: int = 1) -> dict:
    """
    Run p1_Sepsis_hou.py's execute_pipeline with full computational budget.
    ...
    """
    print("\n" + "=" * 100)
    print("SECTION 1 — SEPSIS: ORIGINAL-DATA RUN VIA execute_pipeline()")
...
def run_doubly_null_replicate(
    ...
    This faithfully reproduces p1_Sepsis_hou.py's decision procedure on a log
    ...
    # This matches p1_Sepsis_hou.py: is_significant_final = is_significant_discriminative.
    if m_prime > 0:
        pi0_f, _ = adaptive_storey_pi0(p_hou_m_prime, q=alpha)
        q_hou = storey_qvalue(p_hou_m_prime, pi0_f)
...
def compute_and_save_metrics(
    ...
    results_df = build_rq1_results_df(
        ...
        alpha=ALPHA, log_name="Sepsis",
    )

    print(f"\n  FDR Validation Results (Sepsis, B_null={B_NULL}, double-null protocol):")
    print(f"  {'─'*95}")
    ...
    full_json['null_protocol'] = {
        ...
        'why_label_only_fails': (
            ...
            'Response(ER Triage, IV Liquid) in Sepsis), structural p-values '
    ...
    full_json['alignment_with_p1'] = {
        ...
        'significance_gate': (
            'SINGLE gate: q_Hou <= alpha only. '
            'Matches p1_Sepsis_hou.py is_significant_final = is_significant_discriminative. '
            'Structural evidence is embedded in T_Hou via the W_STRUCT weight. '
    ...
    full_json['config'] = {
        ...
        'N_JOBS': N_JOBS,
        'log': 'Sepsis', 'p1_version': 'p1_Sepsis_hou.py v9.0-HOU-DOUBLY-NULL',
...
def main():
    print("\n" + "=" * 100)
    print("RQ1 — FDR CONTROL VALIDITY: SEPSIS (Imbalanced Stress-Test)")
    print("  Double-Null Protocol: sigma_label ∘ sigma_trace")
    print("  Aligned with p1_Sepsis_hou.py v9.0 (Hou 2005 weighted conjunction, empirical Phipson-Smyth)")
    print("=" * 100)
    ...
    print(f"\n  Decision procedure (faithful to p1_Sepsis_hou.py v9.0):")
    ...
    print(f"\n{'='*100}")
    print("RQ1 — SEPSIS COMPLETE (Double-Null Protocol)")
    print(f"{'='*100}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="RQ1 FDR Control Validity — Sepsis (Double-Null Protocol)"
    )
    parser.add_argument(
