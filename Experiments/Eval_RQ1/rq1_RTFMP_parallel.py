#!/usr/bin/env python3
"""
rq1_RTFMP_parallel.py  —  RQ1 FDR Control Validity: Road Traffic Fine Management Process
=========================================================================================
...
Label definition (Teinemaa et al. TKDE 2019 — canonical RTFM binary outcome):
    Deviant (1): "Send for Credit Collection" present anywhere in case  -> fine escalated
    Normal  (0): "Send for Credit Collection" absent from case          -> fine resolved without collection
...
This is NOT an architectural change to p1_RTFMP.py.  The Phase 1 framework,
...
In RTFMP, the highly stereotyped fine-escalation sequence produces strong
within-class temporal structure visible in any random class partition
(pi0_struct < 1).  The Fisher statistic for those patterns remains shifted rightward:
...
Version : 3.1  (empirical Phipson-Smyth calibration for Storey gate; aligned with p1_RTFMP.py v8.0)
...
from  Experiments.P1_SDSM.p1_RTFMP_parallel import (
# from p1_RTFMP import (
...
RQ1_OUTPUT_DIR = "RQ1_RTFMP_parallel"
...
def run_original_pipeline(n_workers: int = 1) -> dict:
    """
    Run p1_RTFMP.py's execute_pipeline with full computational budget.
...
def run_doubly_null_replicate(
    ...
    This faithfully reproduces p1_RTFMP.py's decision procedure on a log
    ...
    p1_RTFMP.py Step 5b uses Phipson-Smyth empirically-calibrated p-values
...
def compute_and_save_metrics(
    ...
    full_json['null_protocol'] = {
        ...
        'why_label_only_fails': (
            ...
            'ChainSuccession(Create Fine, Send Fine) in RTFMP), structural p-values '
...
def main():
    print("\n" + "=" * 100)
    print("RQ1 — FDR CONTROL VALIDITY: ROAD TRAFFIC FINE MANAGEMENT PROCESS (RTFMP)")
    print("  Double-Null Protocol: sigma_label ∘ sigma_trace")
    print("  Aligned with p1_RTFMP.py v8.0 (empirical Phipson-Smyth Storey gate)")
    print("=" * 100)
    ...
    print(f"\n  Decision procedure (faithful to p1_RTFMP.py):")
