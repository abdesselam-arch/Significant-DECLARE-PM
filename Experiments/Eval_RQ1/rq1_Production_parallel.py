#!/usr/bin/env python3
"""
rq1_Production.py  —  RQ1 FDR Control Validity: Production (Imbalanced Stress-Test)
====================================================================================

PURPOSE
-------
Empirically validate that the Fisher-Storey FDR framework controls the
false discovery rate at nominal alpha = 0.05 on the Production manufacturing
event log, following the gold-standard held-out permutation protocol of
Pellegrina & Vandin (KDD 2018).

Label definition (Teinemaa et al. TKDE 2019 — Production benchmark):
    Deviant (1): label == 'deviant'  -> work order required at least one re-submission
    Normal  (0): label == 'regular'  -> work order completed without re-submission
...
This is NOT an architectural change to p1_Production_parallel.py.  The Phase 1 framework,
...
In Production, patterns with genuine manufacturing temporal regularity
retain structural signal visible in any random class partition.
...
ALIGNMENT WITH p1_Production_parallel.py
-----------------------------------------
...
Version : 3.1  (empirical Phipson-Smyth calibration for Storey gate; aligned with p1_Production_parallel.py v8.0)
...
# from p1_Production_parallel import (
...
def run_original_pipeline(n_workers: int = 1) -> dict:
    """
    Run p1_Production_parallel.py's execute_pipeline with full computational budget.
...
def run_doubly_null_replicate(
    ...
    This faithfully reproduces p1_Production_parallel.py's decision procedure on a log
    ...
    p1_Production_parallel.py Step 5b uses Phipson-Smyth empirically-calibrated p-values
...
def compute_and_save_metrics(
    ...
    full_json['null_protocol'] = {
        ...
        'why_label_only_fails': (
            ...
            'For patterns with genuine within-log temporal regularity in the '
            'Production manufacturing process, structural p-values remain small '
            'in any random class partition. The Fisher statistic is then '
            'T = [non-null chi2(2)] + [null chi2(2)], which is stochastically '
...
def main():
    print("\n" + "=" * 100)
    print("RQ1 — FDR CONTROL VALIDITY: PRODUCTION (Imbalanced Stress-Test)")
    print("  Double-Null Protocol: sigma_label ∘ sigma_trace")
    print("  Aligned with p1_Production_parallel.py v8.0 (empirical Phipson-Smyth Storey gate)")
    print("=" * 100)
    ...
    print(f"\n  Decision procedure (faithful to p1_Production_parallel.py):")
