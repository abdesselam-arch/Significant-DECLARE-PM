...
NULL HYPOTHESIS 2 — H₀ᵈ (Discriminative Null):
    ...
    Purpose:     Directly tests whether the pattern discriminates between
                 Not-Accepted (O_Refused/O_Cancelled) vs. Accepted (O_Accepted) — the primary scientific claim.
...
OUTPUT_DIR = "BPI17_ThreeHyp_SAM_Parallel"
...
class CaseInfo:
    ...
    trace: List[str]               # COMPLETE-lifecycle events, terminal offer activities stripped
...
def load_and_preprocess_data(filepath: str) -> Dict[str, CaseInfo]:
    ...
    Column mapping:
        ...
        case:ApplicationType → loan application type (case-level subgroup dim. 1)
        case:LoanGoal        → loan purpose (case-level subgroup dim. 2)

    Lifecycle filter: Only 'COMPLETE' lifecycle:transition events are retained.
        BPI 2017 records both START and COMPLETE phases for many activities.
    ...
    bpic2017_accepted sub-log definition:
        Step 1: Restrict to O_ (offer-lifecycle) events per case.
        Step 2: Identify the last O_ event by timestamp = terminal offer state.
        Step 3: Label:
            Class 0 (Accepted / Normal)      — terminal O_ event is 'O_Accepted'.
            Class 1 (Not-Accepted / Deviant) — terminal O_ event is 'O_Refused'
                                               or 'O_Cancelled'.
        Cases with no O_ events at all are skipped (no terminal offer state).
    ...
    Outcome-signal removal: Terminal offer activities (O_Accepted, O_Refused,
        ...
        (Init/End/Response at 100% in Class 1 and 0% in Class 0) that encode
        the label rather than genuine process behaviour.
        Non-terminal O_ activities (O_Create Offer, O_Sent (mail and online),
        O_Sent (online only), O_Returned) are retained in the trace.
    """
    print("\n" + "=" * 100)
    ...
    # Terminal offer activities that encode the outcome label — must be stripped
    # from the DECLARE trace to prevent trivially dominant patterns.
    TERMINAL_OFFER_ACTS = {'O_Accepted', 'O_Refused', 'O_Cancelled'}

    case_data = {}
    skipped = {'no_offer_events': 0, 'empty_trace': 0}

    for case_id, group in df.groupby('case:concept:name'):
        case_events = group.sort_values('time:timestamp')

        # ── Label: last O_ event → terminal offer state ──────────────────────
        offer_events = case_events[case_events['concept:name'].str.startswith('O_')]
        ...
        if terminal_offer == 'O_Accepted':
            ...
            # Terminal offer is a non-terminal activity (e.g., O_Create Offer):
            # case has no conclusive offer decision — skip for scientific cleanliness.
            skipped['no_offer_events'] += 1
            continue

        # ── Trace: all COMPLETE events, terminal offer activities removed ─────
        ...
        # ── Case-level subgroup attributes ────────────────────────────────────
...
def extract_subgroups_from_case_data(
    ...
    for cid, case in case_data.items():
        # BPI 2017 subgroups: ApplicationType × LoanGoal
        # Dimension 1: case:ApplicationType (e.g., "New credit", "Limit raise")
        # Dimension 2: case:LoanGoal (e.g., "Home improvement", "Existing loan takeover")
        sgs = []
        if case.application_type not in ('UNKNOWN', '', 'nan', 'None'):
            sg_app = f"AppType_{case.application_type.replace(' ', '_').replace('/', '_')}"
            sgs.append(sg_app)
            subgroup_to_cases.setdefault(sg_app, set()).add(cid)
        if case.loan_goal not in ('UNKNOWN', '', 'nan', 'None'):
            sg_goal = f"LoanGoal_{case.loan_goal.replace(' ', '_').replace('/', '_')}"
            sgs.append(sg_goal)
            subgroup_to_cases.setdefault(sg_goal, set()).add(cid)
        if sgs:
...
def generate_visualizations(
    ...
    summary_text = f"""
    THREE-HYPOTHESIS DISCRIMINATIVE SPECIFICATION MINING — BPI CHALLENGE 2017 SUMMARY
    ══════════════════════════════════════════════════════════════════════════════
