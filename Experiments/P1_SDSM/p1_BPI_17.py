...
NULL HYPOTHESIS 2 — H₀ᵈ (Discriminative Null):
    ...
    Purpose:     Directly tests whether the pattern discriminates between
                 Not-Accepted vs. Accepted — the primary scientific claim.
...
class CaseInfo:
    ...
    trace: List[str]               # Complete-lifecycle events only; terminal offer outcomes stripped
    loan_goal: str                 # case:LoanGoal — stated purpose of the loan (case-level)
    application_type: str          # case:ApplicationType — type of credit application (case-level)
...
def load_and_preprocess_data(filepath: str) -> Dict[str, CaseInfo]:
    ...
    Column mapping:
        ...
        concept:name         → activity name (three namespaces: A_ / O_ / W_)
        lifecycle:transition → event lifecycle state; only 'complete' events are retained
        case:LoanGoal        → stated purpose of the loan (case-level)
        case:ApplicationType → credit application type (case-level)

    Activity namespaces in BPI 2017:
    ...
    All three namespaces are retained in the trace so that DECLARE constraints can
    capture cross-namespace ordering patterns (e.g., A_ → O_ sequencing).

    Outcome determination (binary):
        The label is derived from the LAST O_ (offer) event per case — this is the
        terminal offer state, representing the final disposition of the loan offer.
        Class 0 (Accepted / Normal)     — terminal O_ event == 'O_Accepted'.
        Class 1 (Not-Accepted / Deviant)— terminal O_ event == 'O_Refused' or 'O_Cancelled'.
        Canonical label: Teinemaa et al. (TKDE 2019) — bpic2017_accepted sub-log.
        Skipped — no O_ events in case (cannot determine label), or empty trace after filtering.

    Outcome-signal removal: the three terminal offer outcome activities
    {'O_Accepted', 'O_Refused', 'O_Cancelled'} are stripped from every trace after
    labelling.  Retaining them would produce trivially dominant DECLARE constraints
    (e.g., End(O_Accepted) at 100% in Class 0 and 0% in Class 1) that encode the label
    rather than genuine process behaviour.  These three activities are exclusively
    terminal states — removing them does not alter any intermediate process structure.

    Lifecycle filter: the BPI 2017 log records both 'start' and 'complete' lifecycle
    events.  Only 'complete' rows are retained before building traces; retaining starts
    would duplicate every activity, inflating trace lengths and distorting all pattern
    prevalence estimates.

    Case-level attributes extracted (first non-null value per case):
        case:LoanGoal        → loan_goal        (e.g., 'Personal Loan', 'Home Improvement')
        case:ApplicationType → application_type (e.g., 'New credit', 'Limit raise')
    """
    print("\n" + "=" * 100)
    ...
    # BPI 2017 records both start and complete lifecycle rows per activity.
    # Keeping starts would duplicate every activity in every trace, distorting
    # trace lengths and inflating pattern satisfaction counts.
    ...
    print(f"\n   Activity vocabulary ({len(act_counts)} activities, across A_/O_/W_ namespaces):")
    for act, cnt in act_counts.items():
        print(f"     {act:<45s}: {cnt:,}")

    # Terminal offer outcome activities — used for labelling, then stripped from traces.
    # O_Accepted is the positive outcome (Class 0); O_Refused and O_Cancelled are
    # the two negative terminal states that jointly define Class 1.
    ...
    for case_id, group in df.groupby('case:concept:name'):
        ...
        # Scan from the end to find the last O_ event in the (complete) trace.
        ...
        # ── Outcome-signal removal ───────────────────────────────────────────
        # Strip all three terminal offer outcome activities from the trace.
        # They are terminal states that directly encode the class label; retaining
        # them would produce trivially discriminative Init/End/Response constraints.
        ...
        # case:LoanGoal — stated loan purpose; constant within case
        lg_series = case_events['case:LoanGoal'].dropna() if 'case:LoanGoal' in case_events.columns else pd.Series([], dtype=object)
        loan_goal = str(lg_series.iloc[0]).strip() if len(lg_series) > 0 else 'UNKNOWN'

        # case:ApplicationType — credit application type; constant within case
        ...
        application_type = str(at_series.iloc[0]).strip() if len(at_series) > 0 else 'UNKNOWN'
...
def extract_subgroups_from_case_data(
    ...
    for cid, case in case_data.items():
        # BPI 2017 subgroups: LoanGoal × ApplicationType
        # Dimension 1: case:LoanGoal — stated purpose of the loan application.
        #              Different loan goals (personal, home improvement, car, etc.) follow
        #              distinct offer workflows and have different acceptance rate profiles,
        #              making this a scientifically meaningful process partition.
        # Dimension 2: case:ApplicationType — type of credit application.
        #              New credit applications follow a different process path than limit
        #              raises or renewals; ApplicationType captures this structural split.
        _sentinel = ('UNKNOWN', '', 'nan', 'None', 'none', 'NaN')
        sgs = []
        if case.loan_goal not in _sentinel:
            sg_lg = f"LoanGoal_{case.loan_goal.replace(' ', '_').replace('/', '_')}"
            sgs.append(sg_lg)
            subgroup_to_cases.setdefault(sg_lg, set()).add(cid)
        if case.application_type not in _sentinel:
            sg_at = f"AppType_{case.application_type.replace(' ', '_').replace('/', '_')}"
            sgs.append(sg_at)
            subgroup_to_cases.setdefault(sg_at, set()).add(cid)
        if sgs:
