...
COLORS = {
    'class0': '#D55E00',     # Vermillion — Class 0 (Regular / Normal)
    'class1': '#0072B2',     # Blue — Class 1 (Deviant)
...
OUTPUT_DIR = "../Experiments data/Experiments/Results/Production_ThreeHyp_SAM2"
...
CONFIG = {
    ...
    # Production (m≈varies, n=220, B_null=200): wall-time dominated by holds recomputation.
...
@dataclass
class CaseInfo:
    """Information about a single Production process case."""
    case_id: str
    outcome: int                   # 1 = Deviant (re-submission required), 0 = Regular (Normal)
    trace: List[str]               # Full activity sequence sorted by Complete Timestamp
...
def load_and_preprocess_data(filepath: str) -> Dict[str, CaseInfo]:
    """Load Production process event log and extract case information.

    Column mapping:
        Case ID              → case identifier
        Complete Timestamp   → event timestamp
        Activity             → activity name
        label                → pre-encoded case outcome ('regular' / 'deviant')

    Outcome determination (binary):
        Class 1 (Deviant)         — label == 'deviant': work order required at least one
                                    re-submission. Canonical label: van Dongen (2012).
        Class 0 (Regular / Normal) — label == 'regular': no re-submission required.
        Skipped                    — missing case ID or empty trace.

    No outcome-signal removal is applied: the label is a pre-existing case attribute,
    not an activity that appears in the trace.  The full activity sequence is retained
    as-is, so no trivially dominant patterns are introduced by stripping.

    CSV separator is ';' (semicolon-delimited export from ProM).
    """
    print("\n" + "=" * 100)
    ...
    for case_id, group in df.groupby('Case ID'):
        ...
        trace = case_events['Activity'].tolist()

        if len(trace) == 0:
            skipped['empty_trace'] += 1
            continue

        # Label is a pre-existing case attribute — read from first event row
        label_raw = case_events['label'].iloc[0]
        if pd.isna(label_raw):
            skipped['missing_label'] += 1
            continue
        outcome = 0 if str(label_raw).strip().lower() == 'regular' else 1
...
def extract_subgroups_from_case_data(
    ...
    # Production has no case-level categorical attributes suitable for subgroup analysis.
    del case_data
    return {}, {}
...
def generate_candidate_patterns(
    ...
    candidates_pos = _filter(lpos, "L+ (Deviant)")
    candidates_neg = _filter(lneg, "L− (Regular)")
...
def generate_visualizations(
    ...
    ax.set_xlabel('$\\hat{P}_0$ (Regular)', fontweight='bold')
    ax.set_ylabel('$\\hat{P}_1$ (Deviant)', fontweight='bold')
    ...
    Dataset: {len(case_data):,} cases (Class 0 Regular: {len(D_0):,}, Class 1 Deviant: {len(D_1):,})
