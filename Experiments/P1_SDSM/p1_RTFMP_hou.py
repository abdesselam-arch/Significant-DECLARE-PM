...
CONFIG = {
    ...
    # RTFMP (m≈varies, n=150370, B_null=200): wall-time dominated by holds recomputation.
...
def load_and_preprocess_data(filepath: str) -> Dict[str, CaseInfo]:
    ...
    Outcome determination (binary):
        Class 1 (Sent for Credit Collection / Deviant) — 'Send for Credit Collection'
                                        activity present anywhere in trace.
                                        Canonical label: Teinemaa et al. (TKDE 2019).
        Class 0 (No Credit Collection / Normal) — target activity absent from trace.
        Skipped                         — missing case ID or empty trace.

    Outcome-signal removal: 'Send for Credit Collection' is stripped from the
    activity trace after labelling.  Retaining it would produce trivially dominant
    DECLARE constraints (Init/End/Response at 100% in Class 1 and 0% in Class 0)
    that encode the label rather than genuine process behaviour.

    No lifecycle filter is applied: the RTFMP log records each activity as a
    single completed event, so no START duplicates exist to inflate pattern counts.
    """
    print("\n" + "=" * 100)
    ...
    n0 = len(case_data) - n1
    ir = n0 / n1 if n1 > 0 else float('inf')
...
def extract_subgroups_from_case_data(
    ...
    # RTFMP has no case-level categorical attributes suitable for subgroup analysis.
    return {}, {}
...
def execute_three_hypothesis_protocol(
    ...
    print(f"     Positive (Sent for Credit Collection-dominant): {n_pos:,}")
...
def generate_text_report(
    ...
    rpt.append(f"    Positive (Sent for Credit Collection-dominant): {n_pos:,}")
...
def generate_visualizations(
    ...
    Dataset: {len(case_data):,} cases (Class 0 No-Credit-Collection: {len(D_0):,}, Class 1 Sent-for-Credit-Collection: {len(D_1):,})
